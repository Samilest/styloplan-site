...
export function wallSegments(mask, W, H, opts = {}) {
  ...
  const mh = dropSymbolBridges(splitSparse(mergeParallel(h, true, maxThick), true, mask, W, opts), true, opts);
  const mv = dropSymbolBridges(splitSparse(mergeParallel(v, false, maxThick), false, mask, W, opts), false, opts);
  ...
  return {
    ...
    // The bands that are sparse from end to end, as the openings they are.
    // NOT removed from the walls above: the walls stay exactly as measured,
    // so every gap between them is found as before, and these are added to
    // the gaps by planGaps. See sparseBands for why they exist and symbolGaps
    // for the band they are measured in.
    symbols: [
      ...symbolGaps(sparseBands(mh.walls, true, mask, W, opts), mh.walls, true),
      ...symbolGaps(sparseBands(mv.walls, false, mask, W, opts), mv.walls, false),
    ],
...
/**
 * Mean ink per pixel over a rect of the mask, 0..1.
 */
function fillOf(r, mask, W) {
  const w = r.x1 - r.x0, h = r.y1 - r.y0;
  if (w <= 0 || h <= 0) return 0;
  let n = 0;
  for (let y = r.y0; y < r.y1; y++) for (let x = r.x0; x < r.x1; x++) n += mask[y * W + x];
  return n / (w * h);
}

/**
 * The bands that are sparse from end to end, on a plan whose walls are solid.
 *
 * THE CASE splitSparse CANNOT REACH. It cuts a band where the fill drops for
 * a stretch, and deliberately not at the ends -- so a band with no solid part
 * at all is passed through whole. The Star's porch front has a window drawn
 * as FIVE lines across the wall (a slider: two panels, each a double line,
 * under the face line); the lower four merge into a band 9px thick and 86px
 * long that is 56% ink everywhere. Not thin enough for dropSymbolBridges
 * (9px between 15px walls), not a bridge either (a window gap on one side),
 * and sparse end to end, so splitSparse kept it. It stood as wall: no gap,
 * nothing classified, while the 3D tracer, which erodes thin lines, left the
 * opening empty. Saman circled it -- and the dining room's slider, whose
 * sliding panel is the same kind of band, beside a fixed panel read as a
 * window.
 *
 * WHY THE PLAN DECIDES. A double-line wall is also sparse end to end, and a
 * plan drawn that way must keep every one of them. What separates the two is
 * not the band but the plan: on a solid-walled plan a sparse band is the odd
 * one out. So the share of wall length that is solid is measured first, and
 * only a plan that is mostly solid reports any. The same SPARSE line
 * splitSparse cuts at; the same lesson as densityBand -- the threshold comes
 * from the drawing, not from this file.
 *
 * WHY THEY ARE REPORTED AND NOT REMOVED. Dropping them from the wall list was
 * tried first and measured on every render in the corpus: Plan A lost two
 * windows, because the 312px sparse band between them went and the gap that
 * replaced all three was wider than a gap may be; The Sky (sky3) lost three
 * gaps to the regrouping of its top wall. A wall list that changes changes
 * every gap on that wall. So the walls stay, and the sparse band becomes one
 * more opening beside the gaps -- which is what it is.
 *
 * @returns {Array} the sparse rects, a subset of `rects`
 */
export function sparseBands(rects, horizontal, mask, W, opts = {}) {
  if (!mask || opts.sparseSymbols === false) return [];
  const SPARSE = opts.sparse ?? 0.6;
  const solidShare = opts.solidShare ?? 0.7;
  const len = (r) => (horizontal ? r.x1 - r.x0 : r.y1 - r.y0);
  const fills = rects.map((r) => fillOf(r, mask, W));
  let total = 0, solid = 0;
  rects.forEach((r, i) => { total += len(r); if (fills[i] > SPARSE) solid += len(r); });
  if (!total || solid / total < solidShare) return [];
  return rects.filter((r, i) => fills[i] <= SPARSE);
}

/**
 * A sparse band as a gap, measured in the wall's band and not its own.
 *
 * The five-line window's merged band is 9px of a 15px wall: the face line on
 * top stayed with the wall. Measured in its own 9px it is 56% ink, over the
 * glazing ceiling; in the wall's 15px it is 47%, a window. So the gap takes
 * the band of the wall it sits in -- the union of the collinear neighbours'
 * bands, as collinearGaps does -- and its own band only when it has none.
 *
 * @returns {Array} gap rects carrying `horizontal` and `face`, like collinearGaps'
 */
export function symbolGaps(symbols, walls, horizontal) {
  const cross = (r) => (horizontal ? [r.y0, r.y1] : [r.x0, r.x1]);
  const run = (r) => (horizontal ? [r.x0, r.x1] : [r.y0, r.y1]);
  return symbols.map((s) => {
    const [c0, c1] = cross(s), [a0, a1] = run(s);
    let b0 = c0, b1 = c1;
    for (const w of walls) {
      if (w === s) continue;
      const [wc0, wc1] = cross(w), [wa0, wa1] = run(w);
      const ov = Math.min(c1, wc1) - Math.max(c0, wc0);
      if (ov <= 0 || ov / Math.min(c1 - c0, wc1 - wc0) < 0.5) continue;   // not this wall's line
      if (wa1 < a0 - 40 || wa0 > a1 + 40) continue;                        // not beside it
      // A wall, not a slab: a filled area traced as one wide rect overlaps
      // every band on its way and would hand the symbol a band hundreds of
      // pixels deep (measured: 621px on The Sky). Its own thickness, or so.
      if (wc1 - wc0 > (c1 - c0) * 3) continue;
      b0 = Math.min(b0, wc0); b1 = Math.max(b1, wc1);
    }
    return horizontal
      ? { x0: a0, x1: a1, y0: b0, y1: b1, horizontal: true, face: [b0, b1], symbol: true }
      : { y0: a0, y1: a1, x0: b0, x1: b1, horizontal: false, face: [b0, b1], symbol: true };
  });
}

/**
 * Every opening on the plan: the gaps between collinear walls in both
 * directions, and the sparse bands that are symbols drawn as if they were
 * wall. The one list the readers should build from, so none of them can have
 * the gaps and not the symbols.
 *
 * A symbol that lies over a gap already found is not a second opening. It
 * happens when the band sits a little off its wall's line: the walls either
 * side still bound a gap across it (The Sky's garage door, twice), and one
 * opening framed twice is two panes in one hole.
 *
 * @param {object} seg  from wallSegments
 * @param {object} [opts]  collinearGaps' minGap / maxGap, applied to both
 */
export function planGaps(seg, opts = {}) {
  const minGap = opts.minGap ?? 12, maxGap = opts.maxGap ?? 200;
  const gaps = [
    ...collinearGaps(seg.horizontal, true, opts),
    ...collinearGaps(seg.vertical, false, opts),
  ];
  const run = (g) => (g.horizontal ? [g.x0, g.x1] : [g.y0, g.y1]);
  const cross = (g) => (g.horizontal ? [g.y0, g.y1] : [g.x0, g.x1]);
  const covered = (s) => gaps.some((g) => {
    if (g.horizontal !== s.horizontal) return false;
    const [c0, c1] = cross(s), [gc0, gc1] = cross(g);
    if (Math.min(c1, gc1) - Math.max(c0, gc0) <= 0) return false;
    const [a0, a1] = run(s), [ga0, ga1] = run(g);
    const ov = Math.min(a1, ga1) - Math.max(a0, ga0);
    return ov > 0 && ov / Math.min(a1 - a0, ga1 - ga0) >= 0.5;
  });
  // A symbol that ABUTS a gap on the same line is the rest of that opening,
  // not a second one: The Star's dining slider is a fixed panel the walls
  // stop at (a gap) beside a sliding panel drawn as a sparse band (a symbol),
  // and only together do they read as a slider. The gap grows to take it in.
  const abuts = (s) => gaps.find((g) => {
    if (g.horizontal !== s.horizontal) return false;
    const [c0, c1] = cross(s), [gc0, gc1] = cross(g);
    if (Math.min(c1, gc1) - Math.max(c0, gc0) <= 0) return false;
    const [a0, a1] = run(s), [ga0, ga1] = run(g);
    return Math.abs(a0 - ga1) <= 4 || Math.abs(ga0 - a1) <= 4;
  });
  for (const s of seg.symbols || []) {
    const l = s.horizontal ? s.x1 - s.x0 : s.y1 - s.y0;
    if (l < minGap || l > maxGap || covered(s)) continue;
    const g = abuts(s);
    if (g) {
      const [c0, c1] = cross(s), [gc0, gc1] = cross(g);
      const band = [Math.min(c0, gc0), Math.max(c1, gc1)];
      if (g.horizontal) {
        g.x0 = Math.min(g.x0, s.x0); g.x1 = Math.max(g.x1, s.x1); g.y0 = band[0]; g.y1 = band[1];
      } else {
        g.y0 = Math.min(g.y0, s.y0); g.y1 = Math.max(g.y1, s.y1); g.x0 = band[0]; g.x1 = band[1];
      }
      g.face = band;
      g.joined = true;
      continue;
...
/**
 * The interior openings that carry this plan's window symbol.
 *
 * The exterior test keeps cased openings from being called windows, and it
 * keeps real interior windows empty as well: The Star draws its window
 * symbol on the foyer's wall onto the living room and on the guest bath's,
 * and Saman said a symbol cannot be ignored. So an interior opening is a
 * window when it is drawn the way THIS plan draws its windows:
 *
 *   - the classifier reads it as a window, at a window's width (<= 7ft);
 *   - its glazing weight sits inside the band the plan's own exterior windows
 *     set (densityBand) -- a plan with too few exterior windows to set a band
 *     frames nothing inside, which is the safe side;
 *   - it holds more than one line's worth of ink across the band (1.5 rows):
 *     "a gap SPANNED BY PARALLEL LINES", plural -- the single line a pocket
 *     door's leaf, a stair's stringer or a closet rod draws across a gap is
 *     not the symbol;
 *   - the wall either side of it is solid (solidBeside).
 *
 * Measured over the 33-render corpus with those four: about twenty real
 * windows gained (The Star's two, and windows onto porches and decks that no
 * label opened to the flood), and about seven doorways drawn with two header
 * lines framed by mistake, on five renders. The two forms -- a window drawn
 * as two lines and a cased opening drawn as two lines -- are the same pixels,
 * and no measurement here separates them; the number is stated so the trade
 * is a decision and not a surprise.
 *
 * @param {object} p
 * @param {Uint8Array} p.mask  the ink
 * @param {object} p.seg  from wallSegments
 * @param {Array} p.glazed  from glazedOpenings, the gaps with lines across
 * @param {Array} p.exterior  from exteriorGaps
 * @param {Array} p.exteriorWindows  the exterior gaps classified 'win'
 * @param {number} p.scale  ft per px
 * @returns {Array} the interior gaps to frame, each with `kind: 'win'` and `interior: true`
 */
export function interiorWindows({ mask, W, H, seg, glazed, exterior, exteriorWindows, scale }, opts = {}) {
  const band = densityBand(exteriorWindows.map((g) => bandStats(mask, W, H, g, opts).density), opts);
  if (band.from !== 'plan') return [];
  const isExterior = new Set(exterior);
  const out = [];
  for (const g of glazed) {
    if (isExterior.has(g)) continue;
    const k = classifyOpening(mask, W, H, g, scale, opts);
    if (k.kind !== 'win' || k.widthFt > (opts.winMax ?? 7)) continue;
    const s = bandStats(mask, W, H, g, opts);
    const [f0, f1] = g.face || (g.horizontal ? [g.y0, g.y1] : [g.x0, g.x1]);
    if (s.coverage < (opts.minCoverage ?? 0.8) || s.density < band.lo || s.density > band.hi) continue;
    if (s.density * (f1 - f0) < (opts.minLineRows ?? 1.5)) continue;
    if (!solidBeside(g, seg, mask, W, opts)) continue;
    // A LINE BETWEEN THE FACES, NOT ONLY ON THEM. An interior wall drawn
    // hollow -- two lines at its faces, nothing between -- runs from one
    // solid junction to the next exactly like a two-line window between two
    // jambs ("another": two such walls framed as windows). What a window
    // has that a hollow wall has not is glazing: a line inside the band,
    // clear of both faces.
    if (!innerLine(mask, W, g, opts)) continue;
    out.push({ ...g, kind: 'win', interior: true });
  }
  return out;
}

/**
 * Does a line run along the gap INSIDE the band -- clear of the outer sixth
 * on each side? The glazing line of a window, which a wall drawn as two face
 * lines does not have. A third of the run is enough: a one-pixel line on a
 * light render straddles two rows and thresholds to half its length (run9's
 * kitchen windows: 0.51 and 0.42), while a hollow wall's inner rows measure
 * 0.03-0.07.
 */
export function innerLine(mask, W, gap, opts = {}) {
  const [f0, f1] = gap.face || (gap.horizontal ? [gap.y0, gap.y1] : [gap.x0, gap.x1]);
  const inset = Math.max(1, Math.round((f1 - f0) * (opts.faceInset ?? 1 / 6)));
  const a0 = gap.horizontal ? gap.x0 : gap.y0, a1 = gap.horizontal ? gap.x1 : gap.y1;
  const need = opts.minAlong ?? 0.35;
  for (let c = f0 + inset; c < f1 - inset; c++) {
    let n = 0;
    for (let a = a0; a < a1; a++) n += mask[gap.horizontal ? c * W + a : a * W + c];
    if (n / Math.max(1, a1 - a0) >= need) return true;
  }
  return false;
}

/**
 * Is the gap cut into SOLID wall on both sides?
 *
 * A window symbol on an interior wall is framed only when the wall either
 * side of it is solid ink -- the drawing's own wall weight, not a pair of
 * face lines. The two look-alikes this turns away, measured on the corpus:
 * a pocket door's leaf drawn inside a double-line wall (three lines across a
 * "gap" whose neighbours are two lines with nothing between), and a piece of
 * a deck's outline between two more pieces of the same thin outline. Both
 * carry lines across a gap; neither sits in a wall.
 *
 * @param {object} gap  a gap from planGaps, with `horizontal`
 * @param {object} seg  from wallSegments
 * @param {Uint8Array} mask  the ink
 * @param {object} [opts]  `sparse` (0.6) and `touch` (px of slack at the ends)
 */
export function solidBeside(gap, seg, mask, W, opts = {}) {
  const SPARSE = opts.sparse ?? 0.6, touch = opts.touch ?? 6;
  const rects = gap.horizontal ? seg.horizontal : seg.vertical;
  const run = (r) => (gap.horizontal ? [r.x0, r.x1] : [r.y0, r.y1]);
  const cross = (r) => (gap.horizontal ? [r.y0, r.y1] : [r.x0, r.x1]);
  const [a0, a1] = run(gap), [c0, c1] = cross(gap);
  const beside = (end) => rects.filter((r) => {
    const [rc0, rc1] = cross(r);
    const ov = Math.min(c1, rc1) - Math.max(c0, rc0);
    if (ov <= 0 || ov / Math.min(c1 - c0, rc1 - rc0) < 0.5) return false;
    const [ra0, ra1] = run(r);
    return end === 0 ? Math.abs(ra1 - a0) <= touch : Math.abs(ra0 - a1) <= touch;
  });
  const solid = (rs) => rs.some((r) => fillOf(r, mask, W) > SPARSE);
  return solid(beside(0)) && solid(beside(1));
}
...
/**
 * Cut a merged wall band where it is mostly empty: the window drawn as lines.
 *
 * A plan's commonest window symbol is THREE parallel lines across the wall's
 * own thickness, the outer two on the wall's faces. Row by row those outer
 * lines run straight on from the wall into the symbol and out the other side,
 * so `keepRuns` keeps them, `traceRects` traces them as long thin rects, and
 * `mergeParallel` folds them into one band as thick as the wall and as long
 * as the wall -- continuous through the window. dropSymbolBridges never sees
 * it, because there is no separate thin piece to see. On The Star that hid
 * both bedrooms' front windows and the small ones on the back wall: the
 * tracer (which opens the mask and erases thin lines) had a gap there, this
 * reader had wall, and the opening was neither framed nor closed.
 *
 * What a wall has that the symbol has not is FILL. Along the band, a column
 * through solid wall is all ink; a column through the symbol is three lines
 * in a wall's thickness. So the band is read column by column and cut where
 * the fill drops for at least a short window's width. The cut-out stretch is
 * exactly where the glazing is, and glazedOpenings finds the lines in it as
 * it always did for a window the wall stopped at.
 */
export function splitSparse(rects, horizontal, mask, W, opts = {}) {
  if (!mask) return rects;
  const SPARSE = opts.sparse ?? 0.6;
  const minGap = opts.sparseMinGap ?? 12;
  const out = [];
  for (const r of rects) {
    const a0 = horizontal ? r.x0 : r.y0, a1 = horizontal ? r.x1 : r.y1;
    const c0 = horizontal ? r.y0 : r.x0, c1 = horizontal ? r.y1 : r.x1;
    const thick = c1 - c0;
    if (thick < 3 || a1 - a0 < 3 * minGap) { out.push(r); continue; }
    // fill per position along the run
    const fill = new Float32Array(a1 - a0);
    for (let a = a0; a < a1; a++) {
      let n = 0;
      for (let c = c0; c < c1; c++) n += horizontal ? mask[c * W + a] : mask[a * W + c];
      fill[a - a0] = n / thick;
    }
    // Sparse stretches at least minGap long, wherever they fall. AT THE ENDS
    // TOO: this skipped a stretch touching either end, and The Star's east
    // wall window sat exactly there -- the wall above it is one pixel wider,
    // so the tracer starts a new rect at the window's top edge, and a 127px
    // window at the start of a rect was "an end" and kept as wall. A band
    // that is sparse from end to end is the one shape left alone: it is not
    // this function's to drop (see sparseBands), and cutting it would leave
    // nothing.
    const stretches = [];
    for (let a = a0; a < a1;) {
      if (fill[a - a0] > SPARSE) { a++; continue; }
      let e = a;
      while (e + 1 < a1 && fill[e + 1 - a0] <= SPARSE) e++;
      if (e - a + 1 >= minGap) stretches.push([a, e + 1]);
      a = e + 1;
    }
    if (stretches.length === 1 && stretches[0][0] === a0 && stretches[0][1] === a1) { out.push(r); continue; }
    let start = a0;
    const piece = (p0, p1) => { if (p1 - p0 >= minGap) out.push(horizontal ? { x0: p0, x1: p1, y0: c0, y1: c1 } : { y0: p0, y1: p1, x0: c0, x1: c1 }); };
    for (const [s0, s1] of stretches) { piece(start, s0); start = s1; }
    piece(start, a1);
  }
  return out;
}
 ...
 */
/**
 * WHICH KIND OF WINDOW the symbol draws: fixed, slider or double-hung, and
 * how many mullions.
 *
 * Read from the drafter's own blocks (Guidelines/Windows & Doors, ten DWGs
 * converted with LibreDWG on 2026-09-13; docs/plan-symbols.md, fourth
 * reading) and confirmed on The Star's render, row by row:
 *
 *   fixed         the two face lines and ONE line down the centre, jamb to
 *                 jamb; a mullion is a short tick ACROSS the band where one
 *                 unit meets the next (The Star's 9ft window: ticks at 0.32
 *                 and 0.67 -- three units)
 *   slider        two sashes on two tracks: TWO lines at different depths,
 *                 (a PATIO door is the same symbol at 5.5ft and up, or with
 *                 three or four panels -- see below)
 *                 each a little over HALF the run, meeting in the middle
 *                 (the dining room: [0.00..0.52] on one track, [0.50..0.99]
 *                 on the other)
 *   double-hung   two sashes stacked in depth, each the FULL run: TWO full
 *                 lines between the faces
 *   jalousie      louvres: THREE or more full lines between the faces, close
 *                 together (the drafter's jalousie block, and The Star's
 *                 porch front: a dark band of five lines beside a fixed
 *                 unit, which is how the source plan draws it too)
 *   casement      closed, it is drawn exactly as fixed; open, its sash is a
 *                 line or arc OUTSIDE the wall, which this never sees. Read
 *                 as fixed, and that is the honest answer in plan.
 *
 * The rows of the band are clustered into lines and each line's extent along
 * the run is measured. A line at the band's edge is a face when it spans the
 * run; a partial line at the edge is a sash on the outer track (the slider's
 * right sash sits ON the outer face). Mullions are the columns inked across
 * most of the band, clustered, not at the ends.
 *
 * @param {object} [opts]  `widthFt`, when known, decides slider against patio door
 * @returns {{style:'fixed'|'slider'|'patio'|'double-hung'|'jalousie', mullions:number, sashes:Array<[number,number]>,
 *   units:Array<{lo:number, hi:number, style:string, sashes:Array}>}}
 *   `sashes` are a slider's sash extents as fractions of the run, left to right;
 *   `units` the window's units between its mullions, each with its own style
 */
export function windowStyle(mask, W, H, gap, opts = {}) {
  const horizontal = gap.horizontal;
  const [f0, f1] = gap.face || (horizontal ? [gap.y0, gap.y1] : [gap.x0, gap.x1]);
  const c0 = Math.max(0, Math.round(f0)), c1 = Math.min(horizontal ? H : W, Math.round(f1));
  const a0 = Math.max(0, Math.round(horizontal ? gap.x0 : gap.y0));
  const a1 = Math.min(horizontal ? W : H, Math.round(horizontal ? gap.x1 : gap.y1));
  const thick = c1 - c0, len = a1 - a0;
  const none = { style: 'fixed', mullions: 0, sashes: [], units: [{ lo: 0, hi: 1, style: 'fixed', sashes: [] }] };
  if (thick <= 0 || len <= 0) return none;
  const at = (a, t) => (horizontal ? t * W + a : a * W + t);
  // THE LINES, ONE PER SEGMENT PER ROW, not one per row. A row holds more
  // than one segment when two sashes share a track: the drafter's 4-panel
  // patio door puts its two outer panels on the outer track, [0..0.27] and
  // [0.73..1], and read as one row those are a "full line" and a face.
  // A segment is a run of ink along the row (small breaks allowed: an
  // antialiased line drops a pixel here and there); consecutive rows whose
  // segments lie over each other are one line, thicker.
  const LINE = opts.lineCoverage ?? 0.15;   // a segment shorter than this is a tick, not a line
  const BREAK = Math.max(2, Math.round(len * 0.02));
  const segs = [];   // {t, lo, hi} in run fractions, per row
  for (let t = 0; t < thick; t++) {
    let start = -1, last = -1;
    const flush = () => {
      if (start < 0) return;
      const lo = (start - a0) / len, hi = (last - a0 + 1) / len;
      if (hi - lo >= LINE) segs.push({ t, lo, hi });
      start = -1;
    };
    for (let a = a0; a < a1; a++) {
      if (mask[at(a, c0 + t)]) { if (start < 0) start = a; else if (a - last > BREAK) { flush(); start = a; } last = a; }
    }
    flush();
  }
  const lines = [];   // {t0, t1, lo, hi, span}
  for (const s of segs) {
    const l = lines.find((q) => q.t1 === s.t && Math.min(q.hi, s.hi) - Math.max(q.lo, s.lo) >= 0.8 * Math.min(q.hi - q.lo, s.hi - s.lo));
    if (l) { l.t1 = s.t + 1; l.lo = Math.min(l.lo, s.lo); l.hi = Math.max(l.hi, s.hi); l.span = l.hi - l.lo; }
    else lines.push({ t0: s.t, t1: s.t + 1, lo: s.lo, hi: s.hi, span: s.hi - s.lo });
  }
  lines.sort((p, q) => p.t0 - q.t0 || p.lo - q.lo);
  const rows = { length: thick };
  const FULL = opts.fullSpan ?? 0.85;
  // The faces: the outermost full-span line on each side, when it sits in
  // the outer third of the band. Not "at the very edge": the gap's band is
  // the wall's, a pixel or two wider than the symbol, and a face one row in
  // must still be a face -- while the porch window's second line, three rows
  // in, must not be (it is a sash, and the difference is double-hung).
  const fullLines = lines.filter((l) => l.span >= FULL);
  const topFace = fullLines.length && fullLines[0].t0 < thick * 0.34 ? fullLines[0] : null;
  const bottomFace = fullLines.length && fullLines[fullLines.length - 1] !== topFace && fullLines[fullLines.length - 1].t1 > thick * 0.66 ? fullLines[fullLines.length - 1] : null;
  const inner = lines.filter((l) => l !== topFace && l !== bottomFace);
  // Mullions: columns inked across most of the band, clustered, away from the ends.
  const mullions = [];
  for (let a = a0; a < a1; a++) {
    let n = 0; for (let t = c0; t < c1; t++) n += mask[at(a, t)];
    const f = (a - a0) / len;
    if (n / thick >= (opts.mullionCoverage ?? 0.8) && f > 0.06 && f < 0.94) {
      // A mullion is drawn as its two edges a few px apart: one cluster.
      if (mullions.length && a - mullions[mullions.length - 1].last <= Math.max(3, Math.round(thick / 3))) mullions[mullions.length - 1].last = a;
      else mullions.push({ first: a, last: a });
    }
  }
  // UNITS. A mullion is where one window unit meets the next, and each unit
  // carries its own symbol: The Star's porch front pairs a fixed unit with a
  // double-hung one behind a single jamb. So the run is cut at the mullions
  // and each piece is read on its own; the whole reports the units in order,
  // and its own style is the first unit's.
  if (mullions.length && !opts._unit) {
    const cuts = [0, ...mullions.map((m) => ((m.first + m.last + 1) / 2 - a0) / len), 1];
    const units = [];
    for (let i = 0; i + 1 < cuts.length; i++) {
      const lo = cuts[i], hi = cuts[i + 1];
      if (hi - lo < 0.08) continue;
      const sub = horizontal
        ? { ...gap, x0: a0 + lo * len, x1: a0 + hi * len }
        : { ...gap, y0: a0 + lo * len, y1: a0 + hi * len };
      const u = windowStyle(mask, W, H, sub, { ...opts, _unit: true });
      units.push({ lo, hi, style: u.style, sashes: u.sashes });
    }
    return { style: units[0]?.style ?? 'fixed', mullions: mullions.length, sashes: [], units };
  }
  const unit = (style, sashes) => ({ style, mullions: 0, sashes, units: [{ lo: 0, hi: 1, style, sashes }] });
  // Sashes: a CHAIN of partial lines, each starting where the one before
  // ends (less the overlap where they meet), that together cover the run --
  // two for a slider, three or four for a multi-panel patio door (the
  // drafter's 4-panel block: [0..0.27] out, [0.25..0.52] in, [0.48..0.75]
  // in, [0.73..1] out). Chained rather than paired, and always onto the
  // line that reaches furthest, because the meeting stiles draw a shorter
  // line across the middle (The Star's dining slider: [0.00..0.52],
  // [0.25..0.76], [0.50..0.99]) that is part of the symbol but not a sash.
  const partial = inner.filter((l) => l.span >= 0.15 && l.span < FULL);
  const OVER = opts.sashOverlap ?? 0.3;
  let chain = partial.filter((l) => l.lo <= 0.05).sort((p, q) => q.hi - p.hi).slice(0, 1);
  while (chain.length) {
    const last = chain[chain.length - 1];
    if (last.hi >= 0.95) break;
    const next = partial
      .filter((l) => !chain.includes(l) && l.lo >= last.hi - OVER && l.lo <= last.hi + 0.05 && l.hi > last.hi + 0.1)
      .sort((p, q) => q.hi - p.hi)[0];
    if (!next) break;
    chain.push(next);
  }
  if (chain.length >= 2 && chain[chain.length - 1].hi >= 0.95) {
    // Sashes meet: a break in the drawn line where the stiles cross can
    // leave a sliver between two, and a slider with daylight between its
    // sashes is not a slider.
    const sashes = chain.map((l) => [l.lo, l.hi]);
    for (let k = 1; k < sashes.length; k++) if (sashes[k][0] > sashes[k - 1][1]) sashes[k][0] = sashes[k - 1][1] - 0.02;
    // A SLIDING GLASS DOOR draws exactly the same symbol as a sliding window
    // (the drafter's Sliding-Glass-Door block beside its aluminium-sliding-
    // window block: two panels on two tracks, meeting in the middle). What
    // tells them apart is size: patio doors come in 5, 6, 8, 12ft and three
    // or four panels; sliding windows are 3-5ft and two. So three sashes or
    // more, or two at 5.5ft and up, is a patio door -- glass from the floor.
    const patio = sashes.length >= 3 || (opts.widthFt != null && opts.widthFt >= (opts.patioMinFt ?? 5.5));
    return unit(patio ? 'patio' : 'slider', sashes);
  }
  const full = inner.filter((l) => l.span >= FULL).length;
  if (full >= 3) return unit('jalousie', []);
  if (full === 2) return unit('double-hung', []);
  return unit('fixed', []);
}
...
export function classifyOpening(mask, W, H, gap, ftPerPx, opts = {}) {
  ...
  // WHETHER IT REACHES ACROSS is read on the symbols' ink when the caller
  // has it (window-read.js symbolInk) and the opening carries the window's
  // symbol there -- two lines or more along it (`byLines`): its hairlines,
  // broken into pieces by the ink's cut, still reach. ONE line along an
  // opening is weaker evidence (a wall's edge, a door's frame), and it is
  // read on the ink as it always was: Jordan 2's landing gap has one grey
  // line down it and read as a window the moment that line was whole. HOW
  // MUCH OF THE BAND IS INK stays on the ink, where its ceiling was derived:
  // the symbols' ink draws every line a pixel wider at its edges, and three
  // lines read as a solid bar there (The Avi Top's front window, 41% on the
  // ink and 55% on the other).
  const reach = opts.lines && gap.byLines ? bandStats(opts.lines, W, H, gap, opts).coverage : s.coverage;
  const spans = reach >= (opts.minCoverage ?? 0.8);
  const note = `covers ${s.coverage.toFixed(2)}, density ${(s.density * 100).toFixed(0)}%`
    + (s.returns ? ', jamb returns' : '');

  // THE CEILING IS THREE LINES, NOT A MEASURED SAMPLE. 0.30 was the top of
  // the six windows on hand (3-8%), and The Star's front windows -- the
  // plan's commonest symbol, three parallel lines across the wall's own
  // thickness -- measure 27-34% and were "cased, not glazed". What the symbol
  // can carry is bounded by what it is: at most three lines of about a sixth
  // of the wall's thickness each, half the band. The one thing above that is
  // a solid bar (77% measured), which is not glazing.
  const lightEnough = s.density >= (opts.winDensityMin ?? 0.03)
    && s.density <= (opts.winDensityMax ?? 0.5);

  // The wide band starts where the window band ends, not at the garage door's
  // own minimum: 7-8ft used to fall between the two and land "outside every
  // convention", and ResPlan puts a tenth of all windows there.
  if (widthFt > (opts.winMax ?? 7) && widthFt <= (opts.garageMax ?? 20)) {
    // A SASH CHAIN IS NOT A PANEL. Plan A's front wall carries a 9ft slider
    // -- two half-run lines at two depths meeting in the middle, the symbol
    // the drafter's own block draws (fourth reading) -- and with clean jambs
    // it passed the garage test below as "a panel line across". A garage
    // door reaches across in one piece per row; sashes overlap in the middle
    // and never do. windowStyle already tells them apart, so it is asked
    // first: sashes at glazing density are a sliding unit, and at this width
    // a sliding unit is a patio door (fifth reading, the 5.5ft line).
    if (spans && lightEnough) {
      const style = windowStyle(mask, W, H, gap, { widthFt });
      if (style.style === 'slider' || style.style === 'patio') {
        return tag('win', `${widthFt.toFixed(1)}ft, ${note}, sliding sashes — a patio door`);
      }
      // A MULLION IS NOT ON A GARAGE DOOR EITHER. The Star's 9.1ft bedroom
      // window is two lines with a tick across the middle; no vehicle door
      // block draws a tick, and the jamb-return test that used to keep this
      // window a window reads a different band now (bandFromInk) and no
      // longer fires. The symbol says it; the symbol is asked.
      if (style.mullions >= 1) {
        return tag('win', `${widthFt.toFixed(1)}ft, ${note}, ${style.mullions} mullion(s) — a wide window`);
      }
    }
    // A vehicle door is a panel line across the whole opening, and that is
    // the only thing two renders agree on. The Star draws ONE line with the
    // door's two tracks running from the jambs into the garage -- "returns",
    // exactly as the drafter's overhead-door block (fifth reading) draws it;
    // The Sky draws TWO lines and clean jambs. The rule used to demand no
    // returns, on the strength of a comment, and turned The Star's doors
    // away as "cased" while a porch bay passed; the label rule in view3d
    // covered for it. What a garage door is NOT is the wide window's
    // signature -- glazing lines AND cased jambs together -- so that pair
    // is the one thing refused here. A porch bay between posts (two lines,
    // no returns) still passes, and the GARAGE label still has the last word.
    if (widthFt >= (opts.garageMin ?? 8) && s.coverage >= (opts.garageCoverage ?? 0.9) && !(gap.byLines && s.returns)) {
      return tag('garage', `${widthFt.toFixed(1)}ft, ${note}, a panel line across`);
    }
    // A WIDE WINDOW, in the band a garage door also lives in.
    //
    // ResPlan (2,000 plans, docs/resplan-width-bands-2026-09-12.md): 11% of
    // windows are wider than 7ft, 4% wider than 9ft, 1% wider than 12ft. The
    // cap at 7 missed one window in nine, and width alone cannot fix that,
    // because a single garage door is 8-10ft too. What separates them is the
    // symbol, measured on every wide exterior opening in our own renders:
    //
    //   three garage doors   one panel line across   byLines false  returns
    //   one 9.1ft window     two lines and a mullion byLines true   returns
    //   two porch bays       roof line and slab edge byLines true   no returns
    //   one heavy bar        77% density
    //
    // So in this band a window is what carries the glazing symbol (two or more
    // lines, `byLines` from glazedOpenings), is cased at both jambs (returns,
    // which a porch bay between posts lacks), and is drawn at glazing density.
    // Nothing here is a new number: the density band, the returns test and the
    // symbol test all exist above, and the 12ft ceiling is where ResPlan puts
    // the last percent. Seven openings is a small corpus, and the note says so.
    if (widthFt <= (opts.wideWinMax ?? 12) && gap.byLines && s.returns && spans && lightEnough) {
      return tag('win', `${widthFt.toFixed(1)}ft, ${note}, glazing lines — a wide window`);
    }
    // THE BAND MOVED WITH THE SCALE. Every rule above was measured on plans
    // standing on an assumed 40ft; the door witness (window-read.js,
    // 2026-09-20) found the corpus 15-30% wider than that, and the 6.3ft
    // windows of The Avi's top floor became 7.4ft ones -- the same three
    // glazing lines at the same density, refused here for want of jamb
    // returns, a test the 1.5-7ft band does not consult because it fails on
    // half of Jordan's real windows. Up to 9ft, where ResPlan puts the 96th
    // percentile of windows and below the 10ft a single garage door runs
    // to, a glazed symbol is read as the narrow band reads it. A garage
    // door draws no glazing lines (byLines false, the table above), so it
    // does not enter.
    if (widthFt <= (opts.glazedWinMax ?? 9) && gap.byLines && spans && lightEnough) {
      return tag('win', `${widthFt.toFixed(1)}ft, ${note}, glazing lines`);
    }
 ...
 */
export function sealedOutside(mask, W, H, gaps, opts = {}) {
  ...
  // THE OUTDOORS THE EDGE CANNOT REACH. A covered porch is drawn as a closed
  // figure -- posts, a slab edge, a dashed roof line -- so the flood from the
  // image border stops at its outline and the house wall behind it reads as
  // interior: on The Star, three glazed openings from the living room onto the
  // porch were never classified at all. `opts.seeds` are points the caller
  // knows to be outdoors (see outdoorSeeds: the floor beside a PORCH / DECK /
  // PATIO label); the flood starts from them as well as from the border. A
  // seed on ink is skipped, so a wrong pixel adds nothing.
  for (const s of opts.seeds || []) {
    if (s.x >= 0 && s.x < W && s.y >= 0 && s.y < H) push(s.y * W + s.x);
  }
  while (stack.length) {
    const p = stack.pop();
    const x = p % W, y = (p - x) / W;
...
/**
 * Room names that are outdoors: roofed or not, they have no enclosing walls,
 * and a wall that faces one is an exterior wall. Whole words, so PORCH does
 * not match a room named after somebody, and SUNROOM (enclosed) is not here.
 */
export const OUTDOOR_ROOM = /\b(porch|patio|deck|lanai|verandah?|balcony|terrace|portico|stoop|courtyard|breezeway|loggia|pergola|carport)\b/i;

/**
 * Flood seeds for the outdoor rooms the customer named -- see sealedOutside.
 *
 * The label's own glyphs are ink, so its centre is usually refused. This walks
 * from the centre up, down, left and right and takes the FIRST background
 * pixel each way: the floor between the name and its dimension line, or just
 * past the last letter. Never further, because a porch is shallow and a point
 * that has crossed its back wall would flood the living room as outdoors.
 *
 * @param {Array<{name?:string, x:number, y:number}>} labels  confirmed labels,
 *   x/y as fractions of the render this mask was read from
 * @param {Uint8Array} mask  the ink
 * @param {number} reach  how far to walk, in pixels; a few wall thicknesses
 * @returns {Array<{x:number, y:number}>}
 */
export function outdoorSeeds(labels, mask, W, H, reach) {
  const out = [];
  for (const l of labels || []) {
    if (!OUTDOOR_ROOM.test(l?.name || '')) continue;
    const cx = Math.round(l.x * W), cy = Math.round(l.y * H);
    for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) {
      for (let k = 0; k <= reach; k++) {
        const x = cx + dx * k, y = cy + dy * k;
        if (x < 0 || x >= W || y < 0 || y >= H) break;
        if (!mask[y * W + x]) { out.push({ x, y }); break; }
      }
    }
  }
  return out;
...
export function exteriorGaps(mask, W, H, segments, gaps, opts = {}) {
  ...
  // when `gaps` really is every opening. `opts.seeds` (outdoorSeeds) lets the
  // flood start inside a porch the border cannot reach.
  const outside = sealedOutside(mask, W, H, opts.bridge || gaps, { seeds: opts.seeds });
...
export function glazedOpenings(wall, raw, gaps, opts = {}) {
  ...
  const w = wall.w;
  const h = wall.h ?? Math.floor(raw.mask.length / w);
  const out = [];
  for (const g of gaps) {
    const [f0, f1] = g.face || (g.horizontal ? [g.y0, g.y1] : [g.x0, g.x1]);
    ...
    const at = (a, b) => (g.horizontal ? b * w + a : a * w + b);
    const inside = (a, b) => a >= 0 && b >= 0 && (g.horizontal ? a < w && b < h : a < h && b < w);
    const ink = (a, b) => {
      if (!inside(a, b)) return 0;
      const i = at(a, b);
      return (raw.mask[i] || (raw.low && raw.low[i])) ? 1 : 0;
    };
    // A FRAME DRAWN WIDER THAN THE WALL. Drafters draw a window's frame at
    // the wall's faces, and a styled render as often draws it a little past
    // them: Jordan 4's suite window has its frame 6px outside a 14px wall,
    // so the band held only the two half-length sashes of a slider and the
    // opening read as empty. A line within half the wall's depth of a face
    // counts when it spans the opening and ENDS at its jambs. That is what a
    // frame does and what a floor's grid line or a deck's edge does not:
    // those run on past the opening along the wall.
    const reach = Math.ceil((f1 - f0) / 2);
    const frame = new Set();
    const mid = (a0 + a1) >> 1;
    for (const [lo, hi] of [[f0 - reach, f0], [f1, f1 + reach]]) {
      for (let b = lo; b < hi; b++) {
        if (!ink(mid, b)) continue;
        let s = mid, e = mid;
        while (s > a0 - reach - 1 && ink(s - 1, b)) s--;
        while (e < a1 + reach && ink(e + 1, b)) e++;
        const ends = s >= a0 - reach && e < a1 + reach;
        if (ends && (Math.min(e + 1, a1) - Math.max(s, a0)) / span >= 0.9) frame.add(b);
      }
    }
    const counted = (a, b) => (b >= f0 && b < f1 ? ink(a, b) : frame.has(b) ? ink(a, b) : 0);
    let cols = 0, twoPlus = 0;
    for (let a = a0; a < a1; a++) {
      let runs = 0, prev = 0;
      for (let b = f0 - reach; b < f1 + reach; b++) { const k = counted(a, b); if (k && !prev) runs++; prev = k; }
      cols++;
      if (runs >= 2) twoPlus++;
    }
    let longest = 0;

 ...
 *
 * `tol` is one number, or one per vertex. A chord stands only when every
 * vertex it replaces lies within its OWN tolerance; otherwise it splits where
 * it always did, at the vertex furthest from it. So the splits the textbook
 * rule makes are all made, in the same order, and a smaller tolerance can only
 * add vertices inside a chord the textbook would have let stand. With one
 * number it is the textbook rule exactly.
 */
export function simplify(ring, tol) {
  if (ring.length < 4) return ring;
  const tolAt = Array.isArray(tol) ? (i) => tol[i] : () => tol;
  ...
  while (stack.length) {
    ...
    let worst = -1, at = -1, fits = true;
    for (let i = a + 1; i < b; i++) {
      const [px, py] = ring[i];
      const d = Math.abs(dy * px - dx * py + bx * ay - by * ax) / len;
      if (d > worst) { worst = d; at = i; }
      if (d > tolAt(i)) fits = false;
    }
    if (!fits) { keep[at] = 1; stack.push([a, at], [at, b]); }
  }
  return ring.filter((_, i) => keep[i]);
}

/**
 * The ink's runs at every pixel of it, `h` along the row and `v` down the
 * column (0 off the ink). Across an axis-aligned wall the shorter of the two
 * is the wall's thickness exactly, the same measure the plan's wall floor is
 * taken with (wallFloorPx in extrude.js).
 */
export function inkRuns(mask, W, H) {
  const h = new Uint16Array(W * H), v = new Uint16Array(W * H);
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W;) {
      if (!mask[y * W + x]) { x++; continue; }
      let e = x;
      while (e < W && mask[y * W + e]) e++;
      for (let i = x; i < e; i++) h[y * W + i] = e - x;
      x = e;
    }
  }
  for (let x = 0; x < W; x++) {
...
/** The opening's radius, as a share of the plan's wall floor (wallShapes). */
const OPEN_SHARE = 0.25;

/**
 * The thinnest wall the outlines carry, in pixels, on a plan whose wall floor
 * is `wallPx`. `erode` keeps a pixel standing more than the opening's radius
 * from the ink's edge, and across a band t pixels thick the middle stands
 * ceil(t/2) from it: thinner than this, a wall is pruned as a fin. A wall
 * built on purpose thinner than the floor (a closet drawn in hairlines,
 * extrudeWalls) is built at least this thick, or it is built and never seen.
 */
export const thinnestTraced = (wallPx) => 2 * Math.floor(wallPx * OPEN_SHARE) + 1;

export function wallShapes(mask, W, H, wallPx, opts = {}) {
  const closeR = opts.closeR ?? wallPx * 0.5;
  const openR = opts.openR ?? wallPx * OPEN_SHARE;
  ...
  // A WALL THINNER THAN THE FLOOR IS SIMPLIFIED AT ITS OWN THICKNESS.
  //
  // `tol` is a share of the plan's wall floor, and it keeps a wall's two faces
  // apart only on walls at least that thick (see the table above). A partition
  // drawn thinner than the floor has its faces closer than the tolerance
  // reaches: on Jordan 4 a 3px hall partition, with the floor at 5px, lost
  // both corners of its free end, tapered to nothing over its length, and
  // kept a third of itself in the model; the same wall kept five sixths at a
  // floor of 4. What happened to a wall depended on how thick OTHER walls
  // were, which is the defect. So under the floor the tolerance is the same
  // share of the ink's own thickness, read as the thickest ink within half a
  // floor of the vertex: a pixel of noise on a thick wall's face is still
  // smoothed away as it always was. At or over the floor nothing changes, and
  // `simplify` makes every split it made before (a smaller tolerance only
  // adds vertices), so a plan with no thin wall traces exactly as it did.
  //
  // THE SNAP WAS TRIED THE SAME WAY AND IS NOT DONE. On its own grid a 3px
  // wall on a 7px floor stays 3px, where the plan's grid pulls its faces to
  // 1.75px apart. But a snap MOVES vertices where the tolerance only keeps
  // them, and 25% of a plan's runs are under its floor by definition: a grid
  // per axis moved 34 of 36 renders and took apt413201 over the outline gate;
  // a grid only where no floor-thick ink is near moved 32, thinned a long wall
  // on The Sky by a pixel and tripled another's missed wall. So a thin wall's
  // thickness is still quantised to a quarter of the floor. Measured, that is
  // not what cost Jordan 4 its partition.
  const runs = inkRuns(cleaned, PW, PH);
  const share = (t) => (t > 0 && t < wallPx ? t / wallPx : 1);
  const reach = Math.ceil(wallPx / 2);
  const thickAt = ([x, y]) => {
    ...
    for (let py = cy - reach; py < cy + reach; py++) {
      for (let px = cx - reach; px < cx + reach; px++) {
        ...
        const i = py * PW + px;
        if (runs.h[i]) t = Math.max(t, Math.min(runs.h[i], runs.v[i]));
      }
    }
    return t;
  };
  const tolFor = (ring) => ring.map((p) => tol * share(thickAt(p)));
  ...
  const perimeterOf = (r) => {
    let p = 0;
    for (let i = 0, j = r.length - 1; i < r.length; j = i++) p += Math.hypot(r[i][0] - r[j][0], r[i][1] - r[j][1]);
    return p;
  };
  const usable = (r) => {
    if (r.length < 4) return false;
    if (areaOf(r) < wallPx * wallPx) return false;
    // AND IT HAS TO BE A BAND, NOT A CREASE. A ring that runs out along a line
    // and back along the same line closes with a real area by the shoelace sum
    // and is not a wall.
    //
    // THE OLD TEST COMPARED AREA TO THE RING'S BOUNDING BOX, and that is a test
    // of SHAPE, not of substance. A straight wall fills its box; an L-shaped
    // wall standing on its own -- two legs meeting at a corner, enclosing no
    // room -- fills almost none of it. On The Star the wall between the house
    // and the garage is exactly that: cut off from the rest by the mudroom
    // door on one side and the two garage doors on the other, it is a free
    // L of 16.8 by 22.6ft at 0.36ft thick, 14 sq ft in a 380 sq ft box, 0.036
    // against a floor of 0.05. Every pixel of it was in the mask, in the
    // rectangles and in `walls`, and the model drew nothing there, because
    // the page draws outlines. Saman circled it.
    //
    // Area against perimeter does not know the shape. A band of thickness t
    // and length L has area L*t and perimeter about 2L, so the ratio to the
    // wall's own thickness is about 0.5 whether the band is straight, an L or
    // a U. A crease has area near zero on any perimeter at all. The floor is
    // 0.1: a fifth of a full band, twice as wide a margin as a half-thickness
    // band would need, and nothing a crease can reach.
    const per = perimeterOf(r);
    return per > 0 && wallPx > 0 && areaOf(r) / (per * wallPx) > 0.1;
  };
  ...
  for (let c = 1; c <= n; c++) {
    const rings = contours(label, PW, PH, c)
      ...
      .map((r) => {
        const s = snap(r, grid);
        return simplify(s, tolFor(s)).map(([x, y]) => [x - 1, y - 1]);
      })
      .filter(usable);

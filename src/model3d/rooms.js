...
export function roomGraph({ mask, W, H, segments, gaps = [], doors = [], minRoomPx }) {
  ...
  // Area and box in one pass. The box is what calibration reads: a confirmed
  // label sits inside one of these, and the box's width and height against the
  // label's confirmed dimension is the one measurement on the sheet that says
  // how many pixels a foot is. See calibrate.js.
  const area = new Map();
  const box = new Map();
  for (let p = 0; p < floor; p++) {
    const k = id[p];
    if (k < 0) continue;
    area.set(k, (area.get(k) || 0) + 1);
    const x = p % W, y = (p - x) / W;
    const b = box.get(k);
    if (!b) box.set(k, { x0: x, y0: y, x1: x, y1: y });
    else {
  ...
  return {
    rooms,
    edges,
    // Which room a point is in, by id, or -1. Exposed for calibration, which
    // asks it once per confirmed label.
    roomAt: (x, y) => { const v = at(x, y); return roomIds.has(v) ? v : -1; },
    report: {
      rooms: rooms.length,
      doors: doors.length,

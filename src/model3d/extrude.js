...
import { wallSegments, collinearGaps, sealedOutside, medianWallThickness } from './openings.js';
import { wallShapes, thinnestTraced } from './outline.js';
// The same kind of cycle (window-read.js reaches this file through
// openings.js), used the same way: inside thinClosetWalls only.
import { inkMaskOf, lineLevels, symbolInk } from './window-read.js';
import { findThinClosets } from './closets.js';
const OPEN_RADIUS = 1;
...
// THE WORKING FRAME IS SIZED BY PIXEL BUDGET, NOT BY WIDTH.
//
// It was a fixed 700 wide, whatever the plan's shape. A portrait sheet like
// Jordan (1696x2528) therefore worked in 700x1043, and a wide ranch like The
// Star (2752x1536) in 700x391: the same code, 2.7 times fewer pixels, for the
// wider building. Measured in that frame, Jordan's walls run 15px at the median
// and The Star's run 3, with 17% of its wall runs at 2px or less. The opening
// below erodes by one pixel and so deletes anything two pixels thick: on Jordan
// it cost 26 wall pixels, on The Star 1,084 -- every second interior partition,
// which is what Saman photographed as white lines lying on the floor where
// walls should stand. The survival gate passed at 89%, because the gate is
// global and the casualties were the thin walls.
//
// So every plan now gets the pixels a 700-wide portrait sheet always had, and
// the frame's width follows from its shape. Jordan stays where it was (685
// wide); The Star goes to 1120, where its walls are 4.5px. Never upscaled: a
// small render keeps its own size.
const WORK_PIXELS = 700 * 1000;
export function workingWidth(srcW, srcH) {
  if (!srcW || !srcH) return 700;
  return Math.min(srcW, Math.round(Math.sqrt(WORK_PIXELS * srcW / srcH)));
}
...
export function wallMask(img, opts = {}) {
  ...
  const W = opts.width || workingWidth(srcW, srcH);
  ...
  // A GUARD THAT LIVED HERE FOR ONE DAY, AND WHAT IT TAUGHT. On 2026-09-12 the
  // opening was skipped whenever the tenth-percentile ink run was at or under
  // the brush (2r), on the reasoning that an erosion deletes any run that thin
  // and The Star's partitions were exactly that. Measured across both looks of
  // every render held: on EVERY light render the tenth percentile is 1, because
  // fixture lines, text and hatching are one-pixel dark ink on a light sheet,
  // so the opening was skipped on all of them and every fixture stroke became
  // a wall -- Plan A went from 72 wall pieces to 958, and The Sky's light trace
  // drifted 28% in aspect from its dark one, which made the floor borrow from
  // the dark render under a light model. And on The Star itself, once the
  // frame above was sized by budget, the tenth percentile read 4 and the guard
  // never fired: the budget was the fix, the guard fixed nothing and broke
  // everything else. A statistic over ALL thin ink is not a statistic about
  // thin WALLS. The opening runs as it always did; the frame is what protects
  // a thin wall now.
  ...
  // HATCHING IS NOT WALL. An outdoor deck, a porch or a bay is drawn as a
  // fill of parallel diagonal strokes, and the styling paints it a mid tone
  // that belongs in `low` -- drawn, not wall. On most renders it is darker
  // than the wall threshold and never reaches `solid`; on Jordan 4 the model
  // drew it lighter (a median of 203 on the deck against 177-180 on The Sky
  // and The Avi, with the threshold at 190), its bright strips passed as wall
  // ink, and the gap closing and the wall rescue below welded them into
  // blocks as tall as the walls: both decks and both bays (Saman, 2026-09-24,
  // the bug list). No threshold moves: fillRegions decides by what the ink
  // IS, so a lighter or darker hatch on the next render is read the same way.
  //
  // ONLY THE FILL MOVES. The drawing's line-weight statistics (wallFloorPx,
  // which sizes the outline stage's smoothing) are still taken from the ink
  // as drawn, `inkAsDrawn` below: removing the hatch raised Jordan 4's floor
  // from 4px to 5px, and the outline stage then lost most of a 3px partition
  // beside the stairs that it had kept before. That stage's handling of
  // walls thinner than the floor is its own question, measured across the
  // corpus before it is touched; this fix takes the fill out and nothing
  // else. On every render without a fill the two masks are the same array.
  let inkAsDrawn = solid;
  const fill = fillRegions(solid, lum, inverted, W, H);
  if (fill.some((v) => v)) {
    inkAsDrawn = solid.slice();
    for (let p = 0; p < W * H; p++) if (fill[p]) { solid[p] = 0; low[p] = 1; }
  }
  // A WINDOW IS WALL IN SECTION. `opts.fill` is a list of rectangles in SOURCE
  // pixels -- the exterior openings the page has read as glazed, see
  // windowFills in view3d.html -- and each is painted solid here, after the
  // opening so the brush cannot erase what was just put in. On the render a
  // window is the wall stopping and thin lines crossing, and the lines are too
  // thin to be wall, so without this a window is a gap indistinguishable from
  // a doorway and a wall that is mostly windows reads as no wall. Six ways of
  // drawing something in the gap were rejected on look; this is the drawing's
  // own reading -- a plan is a horizontal section and it shows a window because
  // the cut passes through glass -- and the symbol stays visible on the floor.
  if (opts.fill?.length) {
    const kx = W / srcW, ky = H / srcH;
    for (const f of opts.fill) {
      const x0 = Math.max(0, Math.floor(f.x0 * kx)), x1 = Math.min(W, Math.ceil(f.x1 * kx));
      const y0 = Math.max(0, Math.floor(f.y0 * ky)), y1 = Math.min(H, Math.ceil(f.y1 * ky));
      for (let y = y0; y < y1; y++) for (let x = x0; x < x1; x++) { solid[y * W + x] = 1; inkAsDrawn[y * W + x] = 1; }
    }
  }
  // A pixel cannot be both; the opening can grow one into the other.
  for (let p = 0; p < W * H; p++) if (solid[p]) low[p] = 0;

  return { mask: solid, low, w: W, h: H, inverted, inkAsDrawn: inkAsDrawn === solid ? solid : inkAsDrawn };
}

/**
 * The pixels of the wall mask that are a FILL -- a deck's or a bay's hatch --
 * and not wall.
 *
 * WHAT A WALL IS, IN THE MASK: a band, so every wall pixel lies on a long run
 * along one axis, or close to one. The plan's own wall thickness T is
 * measured (medianWallThickness); a run of 3T along an axis is a band. A
 * hatch is diagonal strokes, broken into dots at the working resolution: no
 * long axis run anywhere in it. Measured on every dark render (2026-09-24),
 * the solid pixels with no band within 2T of them: 27,354 on Jordan 4 (its
 * decks and bays), and 0 to 1,044 on the other 25, every one of them a post
 * or a speck -- small and compact.
 *
 * SO A FILL IS AN AREA, NOT A BLOB: those band-less pixels, gathered into
 * clusters, and a cluster more than 4T across in BOTH directions is a filled
 * area, not a post (a post is one or two wall thicknesses) and not a wall
 * (a wall is a band). Measured, the posts sit at or under 3T and Jordan 4's
 * fills at 10T and more.
 *
 * AND THE EDGE OF THE FILL, next to a wall, goes with it by TONE, not by
 * distance: within the fill's box the ink that is not wall band and is
 * darker than halfway between the wall's own tone (the median of the band
 * pixels) and the fill's (the median of its band-less pixels) is fill. A
 * nib between two openings is wall-white and short, and stays; a hatch dot
 * against the wall is the hatch's tone, and goes.
 *
 * @returns {Uint8Array} 1 where a mask pixel is fill
 */
export function fillRegions(mask, lum, inverted, W, H) {
  const N = W * H;
  const out = new Uint8Array(N);
  const T = Math.max(2, medianWallThickness(mask, W, H));
  const h = new Uint16Array(N), v = new Uint16Array(N);
  for (let y = 0; y < H; y++) { let x = 0; while (x < W) { if (!mask[y * W + x]) { x++; continue; } let e = x; while (e < W && mask[y * W + e]) e++; for (let k = x; k < e; k++) h[y * W + k] = e - x; x = e; } }
  for (let x = 0; x < W; x++) { let y = 0; while (y < H) { if (!mask[y * W + x]) { y++; continue; } let e = y; while (e < H && mask[e * W + x]) e++; for (let k = y; k < e; k++) v[k * W + x] = e - y; y = e; } }
  const band = new Uint8Array(N);
  for (let p = 0; p < N; p++) if (mask[p] && Math.max(h[p], v[p]) >= 3 * T) band[p] = 1;
  // band pixels within 2T: a summed-area table
  const S = new Int32Array((W + 1) * (H + 1));
  for (let y = 0; y < H; y++) { let row = 0; for (let x = 0; x < W; x++) { row += band[y * W + x]; S[(y + 1) * (W + 1) + x + 1] = S[y * (W + 1) + x + 1] + row; } }
  const R = 2 * T;
  const near = (x, y) => {
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    const p = y * W + x;
    if (mask[p] && !band[p] && near(x, y) === 0) { core[p] = 1; any = true; }
  }
  if (!any) return out;
  // Clusters: core pixels within T of each other are one cluster (a hatch is
  // dots; a post is one blob).
  const label = new Int32Array(N);
  const boxes = [];
  const stack = [];
  for (let s = 0; s < N; s++) {
    if (!core[s] || label[s]) continue;
    const id = boxes.length + 1;
    const b = { x0: W, y0: H, x1: 0, y1: 0, px: [] };
    ...
    while (stack.length) {
      ...
      for (let dy = -T; dy <= T; dy++) for (let dx = -T; dx <= T; dx++) {
        ...
        const q = Y * W + X;
        if (core[q] && !label[q]) { label[q] = id; stack.push(q); }
      }
    }
    boxes.push(b);
  }
  const fills = boxes.filter((b) => b.x1 - b.x0 + 1 > 4 * T && b.y1 - b.y0 + 1 > 4 * T);
  if (!fills.length) return out;
  // ink strength: brighter on a dark render, darker on a light one
  const ink = (p) => (inverted ? lum[p] : 255 - lum[p]);
  const median = (vals) => { vals.sort((a, b) => a - b); return vals.length ? vals[vals.length >> 1] : 0; };
  const wallVals = [];
  for (let p = 0; p < N; p++) if (band[p]) wallVals.push(ink(p));
  const wallTone = median(wallVals);
  for (const b of fills) {
    const cut = (wallTone + median(b.px.map(ink))) / 2;
    const x0 = Math.max(0, b.x0 - R), y0 = Math.max(0, b.y0 - R), x1 = Math.min(W - 1, b.x1 + R), y1 = Math.min(H - 1, b.y1 + R);
    for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
      const p = y * W + x;
      if (mask[p] && !band[p] && ink(p) < cut) out[p] = 1;
    }
    for (const p of b.px) out[p] = 1;
  }
  return out;
}
...
export function wallFloorPx(mask, W, H) {
  return runPercentile(mask, W, H, 0.25);
}

/**
 * The length of ink runs across the mask, both sweeps, at one percentile.
 * The quarter mark is the plan's wall floor (wallFloorPx); the tenth is what
 * the opening's brush is checked against.
 */
export function runPercentile(mask, W, H, q) {
  const runs = [];
...
export function detailTexture(img, m, bounds, scale = 3, opts = {}, drawnBox = null,
  ...
  // READ BACK IN MEMORY, NOT FROM THE GPU. The passes below read every pixel
  // and write it again; on a GPU-backed canvas each read is a transfer out
  // of video memory, and the browser says so in the console. Measured
  // 2026-09-20 on four renders, both looks (test/texture-probe.html), with
  // the one-trip and stepped-over loops below: identical pixels, a texture
  // in 5.0s across the eight instead of 7.5s -- a third less.
  ...
  // `paintOut: false` leaves the walls in: the PLAN texture the flat view
  // lays over the model once the walls are down (view3d's 2D switch), which
  // is the render as the customer paid for it, lines and all.
  const alsoLow = opts.detectWindows && m.low;
  ctx.fillStyle = floor;
  if (opts.paintOut !== false) {
  ...
  // depths came out 41% off, so it is written once and used once: a texture
  // column maps to one mask column and a texture row to one mask row, so the
  // two lookups are tabulated here and the pass below reads them, rather
  // than rounding twice for each of the texture's four million pixels
  // (measured 2026-09-20: that closure was a third of the paper lift).
  // Off the mask, in either axis, is open air (-1).
  const airAt = air
    ? {
      air, mw: m.w,
      col: Int32Array.from({ length: out.width }, (_, tx) => { const mx = Math.round(bounds.x0 + tx / scale); return mx < 0 || mx >= m.w ? -1 : mx; }),
      row: Int32Array.from({ length: out.height }, (_, ty) => { const my = Math.round(bounds.y0 + ty / scale); return my < 0 || my >= m.h ? -1 : my; }),
    }
    : null;
  // ONE READ AND ONE WRITE of the texture for both passes. Each pass read the
  // whole canvas back and wrote it again, ~19MB each way on a 2528px render;
  // the pixels between the two trips were the same pixels.
  const px = ctx.getImageData(0, 0, out.width, out.height);
  // WHICH PIXELS ARE PAPER, remembered. The paper the lift moves toward the
  // background is not a surface: it must neither veil nor hide a stair that
  // descends below the sheet beside it (a porch's steps, seen from the side,
  // wore the paper's rim as a smear of shadow -- Saman, 2026-09-21). The deck
  // and the floor ARE surfaces and keep hiding what lies under their rim.
  // The engine draws depth from this mask and colour from the canvas.
  const liftedAt = new Uint8Array(out.width * out.height);
  const lifted = liftPaper(px, inner, opts.sheetBg, airAt, liftedAt);
  const faded = fadeSheetEdge(px, inner, opts.sheetBg);
  if (lifted || faded) ctx.putImageData(px, 0, 0);
  // The occluder: white where the sheet is a surface, black where it is
  // lifted paper or nothing. A quarter of the texture's size is plenty for
  // a depth mask, and a quarter of its memory.
  const occ = document.createElement('canvas');
  const ow = Math.max(1, Math.round(out.width / 4)), oh = Math.max(1, Math.round(out.height / 4));
  occ.width = ow; occ.height = oh;
  const og = occ.getContext('2d', { willReadFrequently: true });
  const od = og.createImageData(ow, oh), odd = od.data, pd = px.data;
  for (let y = 0; y < oh; y++) for (let x = 0; x < ow; x++) {
    const sx = Math.min(out.width - 1, x * 4 + 2), sy = Math.min(out.height - 1, y * 4 + 2);
    const sp = sy * out.width + sx;
    const surface = pd[sp * 4 + 3] > 0 && !liftedAt[sp];
    const o = (y * ow + x) * 4;
    odd[o] = odd[o + 1] = odd[o + 2] = surface ? 255 : 0; odd[o + 3] = 255;
  }
 ...
 */
/**
 * @param {ImageData} img  the texture's pixels, lifted in place
 * @param {{x0,y0,x1,y1}} inner  the drawing's box, in texture pixels
 * @param {number[]|null} sheetBg  the scene's ground, or nothing to lift toward
 * @param {{air:Uint8Array, mw:number, col:Int32Array, row:Int32Array}|null} airAt
 *   the open air around the building, with the texture column -> mask column
 *   and row -> mask row lookups (-1 is off the mask, which is air)
 * @returns {boolean} whether anything was written
 */
function liftPaper(img, inner, sheetBg, airAt, liftedAt = null) {
  ...
  if (ring.length < 16) return false;
  ...
  const out = [0, 0, 0], px = [0, 0, 0];
  const air = airAt?.air, col = airAt?.col, row = airAt?.row, mw = airAt?.mw;
  for (let y = 0, i = 0; y < H; y++) {
    // A texture row off the mask is air in whole; on it, one mask row.
    const my = airAt ? row[y] : -1;
    const base = my < 0 ? -1 : my * mw;
    for (let x = 0; x < W; x++, i += 4) {
      if (d[i + 3] !== 255) continue;
      if (airAt && base >= 0) {
        const mx = col[x];
        if (mx >= 0 && !air[base + mx]) continue;
      }
      px[0] = d[i]; px[1] = d[i + 1]; px[2] = d[i + 2];
      liftPixel(px, paper, bg, near, out);
      // remembered as paper when the lift moved it at all
      if (liftedAt && (out[0] !== px[0] || out[1] !== px[1] || out[2] !== px[2])) liftedAt[i >> 2] = 1;
...
/**
 * @param {ImageData} img  the texture's pixels, faded in place
 * @param {{x0,y0,x1,y1}} inner  the drawing's box, in texture pixels
 * @param {number[]|null} sheetBg  the scene's ground
 * @returns {boolean} whether anything was written
 */
function fadeSheetEdge(img, inner, sheetBg) {
  ...
  const bg = Array.isArray(sheetBg) ? sheetBg : null;
  const W = img.width, H = img.height;
  if (inner.x0 <= 0 && inner.y0 <= 0 && inner.x1 >= W && inner.y1 >= H) return false;
  const d = img.data;
  const left = Math.max(1, inner.x0), top = Math.max(1, inner.y0);
  const right = Math.max(1, W - inner.x1), bottom = Math.max(1, H - inner.y1);
  // The fade lives in the margin, and the margin is a few percent of the
  // sheet on each side: inside the drawing's own rows, the run between its
  // left and right edge is untouched and is stepped over rather than visited.
  const skipFrom = Math.max(0, inner.x0), skipTo = Math.min(W, inner.x1);
  for (let y = 0; y < H; y++) {
    const oy = y < inner.y0 ? (inner.y0 - y) / top
      : y >= inner.y1 ? (y - inner.y1 + 1) / bottom : 0;
    const drawnRow = oy <= 0 && skipTo > skipFrom;
    for (let x = 0; x < W; x++) {
      if (drawnRow && x === skipFrom) { x = skipTo; if (x >= W) break; }
...
/**
 * A STUB OF WALL IS A WALL, however short and lean the drawing made it.
 *
 * The size test above drops a piece that is both thin (under 0.25ft) and
 * short (under 3ft), and that is right for dust: specks and slivers lying
 * loose or along a wall's face. It is wrong for the short returns a closet
 * front or a doorway is built from. Jordan 4 draws its bedroom closet's two
 * front walls 1.4 and 1.6ft long and 3px thick on a 4px floor; both went,
 * the closet had no mouth for its doors to hang in, and its front stood
 * open. A constant in feet cannot tell the two apart (0.25ft is 3.5px on
 * that render and 12px on another); the SHAPE can.
 *
 * A stub grows out of a wall END-ON: the ink continues past one of its ends
 * and not the other, and nothing lies along either of its long sides. A
 * sliver beside a wall has ink along a long side (Jordan 3's widened wall
 * end, which keeping by thickness alone turned into a phantom doorway);
 * a speck has ink at neither end. It must still be at least half the
 * plan's own thinnest wall thick, so a one-pixel line drawn out of a wall is
 * not one, and as long as the absolute floor. A leaf drawn as a bar in the
 * wall's tone has the same shape, and the engine already stands none where
 * a door hangs (isLeafBar).
 *
 * @param {Uint8Array} mask  the sealed wall mask the pieces were cut from
 * @param {number} floorPx   the plan's wall floor (wallFloorPx)
 * @param {number} minLongPx the absolute floor, in pixels
 */
export function isWallStub(mask, W, H, floorPx, minLongPx) {
  const inkAt = (x, y) => x >= 0 && y >= 0 && x < W && y < H && mask[y * W + x] === 1;
  const share = (n, at) => {
    let c = 0;
    for (let i = 0; i < n; i++) if (inkAt(...at(i))) c++;
    return n ? c / n : 0;
  };
  return (r) => {
    const horizontal = r.x1 - r.x0 >= r.y1 - r.y0;
    const thick = horizontal ? r.y1 - r.y0 : r.x1 - r.x0;
    const long = horizontal ? r.x1 - r.x0 : r.y1 - r.y0;
    if (thick < floorPx / 2 || long < minLongPx) return false;
    // Just past each end, across the thickness; and just outside each long side.
    const end = (at) => share(thick, (t) => (horizontal ? [at, r.y0 + t] : [r.x0 + t, at]));
    const side = (at) => share(long, (l) => (horizontal ? [r.x0 + l, at] : [at, r.y0 + l]));
    const e0 = end(horizontal ? r.x0 - 1 : r.y0 - 1) >= 0.5;
    const e1 = end(horizontal ? r.x1 : r.y1) >= 0.5;
    const s0 = side(horizontal ? r.y0 - 1 : r.x0 - 1);
    const s1 = side(horizontal ? r.y1 : r.x1);
    return e0 !== e1 && s0 < 0.5 && s1 < 0.5;
  };
}

// The symbols' ink of a picture, kept per image: the reader of a floor traces
// it two or three times (readPlan), and the picture does not change. Only an
// <img> is kept, and only for its own source; a canvas can be redrawn.
const symbolInkMemo = new WeakMap();
function symbolInkOf(img) {
  const keep = typeof HTMLImageElement !== 'undefined' && img instanceof HTMLImageElement;
  const had = keep && symbolInkMemo.get(img);
  if (had && had.src === img.currentSrc + '|' + img.src) return had;
  const { ink, w, h, pos } = inkMaskOf(img);
  const rec = { lines: symbolInk(ink, pos, lineLevels(ink, pos, w, h), w), w, h, src: keep ? img.currentSrc + '|' + img.src : '' };
  if (keep) symbolInkMemo.set(img, rec);
  return rec;
}

/**
 * THE CLOSETS DRAWN IN THIN LINES (closets.js findThinClosets), as walls.
 *
 * A closet the plan draws as a box of hairlines -- Plan A's beside BATH 2 --
 * has no wall ink for the tracer to build, so its sides were never built and
 * its doors hung in the hall. It is read on the picture's own pixels, where a
 * hairline is a line, against the walls already kept, and handed back in this
 * mask's pixels: the partitions to add, and each closet's mouth, normalised
 * to the picture, for the reader to list as the doorway it is.
 *
 * @returns {{partitions:Array<{x0,y0,x1,y1}>, mouths:Array}}
 */
function thinClosetWalls(img, m, px, floorPx, keptPx) {
  const { lines, w, h } = symbolInkOf(img);
  const sx = m.w / w, sy = m.h / h;
  const grid = new Uint8Array(m.w * m.h);
  for (const r of keptPx) for (let y = r.y0; y < r.y1; y++) grid.fill(1, y * m.w + r.x0, y * m.w + r.x1);
  const wallAt = (x, y) => {
    const X = Math.floor(x * sx), Y = Math.floor(y * sy);
    return X >= 0 && Y >= 0 && X < m.w && Y < m.h && grid[Y * m.w + X] === 1;
  };
  // Drawn thinner than the outlines can carry, a side is built as thin as
  // they can (thinnestTraced), grown away from the closet.
  const found = findThinClosets(lines, w, h, { ftPerPx: px * sx, wallPx: floorPx / sx, wallAt, minThick: thinnestTraced(floorPx) / sx });
  // Marked, because they are no sample of the plan's own wall thickness
  // (wallsFromReading measures that on the rest).
  const toMask = (q) => ({
    x0: Math.max(0, Math.floor(q.x0 * sx)), y0: Math.max(0, Math.floor(q.y0 * sy)),
    x1: Math.min(m.w, Math.ceil(q.x1 * sx)), y1: Math.min(m.h, Math.ceil(q.y1 * sy)),
    closet: true,
  });
  return {
    partitions: found.flatMap((k) => k.partitions.map(toMask)),
    mouths: found.map((k) => ({ x0: k.mouth.x0 / w, x1: k.mouth.x1 / w, y0: k.mouth.y0 / h, y1: k.mouth.y1 / h, horizontal: k.mouth.horizontal })),
  };
}
...
export function extrudeWalls(img, widthFt, opts = {}) {
  ...
  // From the ink as drawn, before any fill was taken out (wallMask).
  const floorPx = wallFloorPx(m.inkAsDrawn || m.mask, m.w, m.h);
  ...
  // THE PLAN, WALLS AND ALL, for the flat view -- the same crop, the same
  // paper lift and edge fade, nothing painted out. Built only when asked (it
  // is a second full texture, ~1100ms on The Sky) and kept once built; a
  // function on the result, so it never travels in a published record.
  let planCanvas = null;
  const planFloor = () => (opts.floor === false ? null
    : (planCanvas ||= detailTexture(img, m, sheet, opts.textureScale, { ...opts, paintOut: false }, content,
      sheetAir(sealed, m.w, m.h))));

  // A PUBLISHED READING, USED IF IT IS A READING THIS ENGINE CAN DRAW.
  //
  // The mask dimensions are derived from the image, so equal dimensions mean
  // the same picture at the same scale. The version used to have to be THIS
  // version -- "a stale record can never show a visitor a building we would
  // not draw ourselves" -- and the measurement of 2026-09-20 is why it no
  // longer does: every bump of GEOMETRY_VERSION (nine this week, each a
  // better reading) sent every visitor of every published floor down the
  // full analysis below, 25 seconds on a desktop for a v21 floor, longer on
  // a phone, until the builder republished. What the builder published is
  // the building they looked at and approved; a fresher reading is theirs
  // to publish (cloud-publish.js marks the floor behind), not the visitor's
  // to wait for. A record is drawable from the version that first carried
  // openings with their kinds and the ink rectangles the outlines are cut
  // from (MIN_SHIPPED_VERSION); older ones -- two test floors from the first
  // week -- still fall through to the analysis, which is what they always
  // did. A field the engine reads that an older record lacks reads as
  // absent, never as wrong: a leaf without its angle hangs at a quarter.
  const shipped = opts.geometry;
  if (shipped && shipped.version >= MIN_SHIPPED_VERSION && Array.isArray(shipped.openings) && shipped.inkRects?.length
    ...
    const reshape = (openings) => (openings?.length && shipped.inkRects
      ...
      : (shipped.shapes || []));
    return {
      ...shipped, reshape, inverted: m.inverted, floor: floorTexture(), planFloor, fromPublished: true,
    };
  }
  ...
  const stub = isWallStub(sealed, m.w, m.h, floorPx, (opts.minWallFt ?? 0.5) / px);
  const keptPx = allPx.filter((r) => {
    if (sized({ x0: 0, z0: 0, x1: (r.x1 - r.x0) * px, z1: (r.y1 - r.y0) * px })) return true;
    return localWallPx(dist, m.w, r) >= floorPx * 0.99 || stub(r);
  });
  ...
  // A closet drawn in thin lines, built as drawn (thinClosetWalls): its sides
  // are as thin as the plan draws them, so, like a bridge, never widened.
  const thin = opts.closets === false ? { partitions: [], mouths: [] } : thinClosetWalls(img, m, px, floorPx, keptPx);
  const walls = thickenStubs(keptPx.map(toFt), floorFt, 3, inkThickFt)
    .concat(bridges.map(toFt), thin.partitions.map(toFt));
  ...
  const frame = { bounds, px, floorPx, toX, toZ };
  const inkRects = keptPx.concat(bridges, thin.partitions);
  ...
  return {
    ...
    inkRects,
    // The doorway of each closet drawn in thin lines, normalised to the
    // picture: its sides are walls now, and no gap finder lists the opening
    // between their ends (planFromReading does).
    closetMouths: thin.mouths,
...
export function traceOutlines(inkRects, frame, openings) {
  const { bounds: extentBounds, px, floorPx, toX, toZ } = frame;
  // THE RASTER HOLDS EVERY KEPT PIECE, NOT ONLY THE BUILDING'S BOX.
  //
  // `bounds` is the extent: set by pieces spanning at least 4% of the plan, so
  // that a post showing in one look and not the other cannot change the
  // building's shape between them (see the note on the extent). It was also
  // the window this raster was cut to -- and a deck post standing outside the
  // exterior wall, kept by the size test and present in `inkRects`, fell off
  // the raster's edge and was never traced. Saman marked twenty-seven of them
  // across six plans. The extent keeps its rule; the raster is the union of
  // the extent and everything kept, and the model's coordinates simply run a
  // little past the extent where a post does.
  ...
  // A CLOSET'S SIDE IS NEVER A DOORWAY'S. A closet drawn in hairlines opens
  // at its mouth, which is not one of its sides (thinClosetWalls), and a
  // side built as thick as the outlines can carry may reach into the band of
  // a doorway beside it -- Plan A's closet top, under the doorway above it,
  // was cut down to a fin and pruned.
  if (openings?.length) {
    for (const r of inkRects) {
      if (!r.closet) continue;
      for (let y = Math.max(r.y0, bounds.y0); y < Math.min(r.y1, bounds.y1); y++) {
        for (let x = Math.max(r.x0, bounds.x0); x < Math.min(r.x1, bounds.x1); x++) {
          ink[(y - bounds.y0 + 1) * bw + (x - bounds.x0 + 1)] = 1;
        }
 ...
 *
 * 9: the width a reading is traced at is calibrated from the confirmed rooms
 *    (calibrate.js) instead of assumed to be 40ft. Nothing in this file moved;
 *    the INPUT did, and a reading traced at 40 on an 80ft plan has half its
 *    closet doors sealed and its short walls dropped. Refusing those records
 *    sends the visitor to a local trace, which now calibrates itself.
 * 10: the working frame is sized by pixel budget rather than a fixed 700
 *    width, and the opening is skipped when the plan's thin walls would not
 *    clear its brush. See workingWidth. A wide plan traced at v9 is missing
 *    every partition that came out two pixels thick.
 * 11: exterior openings read as glazed are painted into the mask as wall
 *    before tracing (`opts.fill`, see windowFills in view3d.html). A reading
 *    without them has every window as a full-height gap.
 * 12: outline.js no longer drops a free-standing L- or U-shaped wall as a
 *    "crease". A reading traced at 11 is missing the house/garage wall on
 *    any plan where doors cut it off from the rest.
 * 15: the record carries `openings` -- every window and garage door with its
 *    kind and its units, in the render's pixels (window-read.js) -- and the
 *    viewer frames a visitor's windows from them. A record without the field
 *    would show a visitor bare gaps where the builder sees glass.
 * 16: the openings are read BETWEEN THIS RECORD'S OWN WALLS (window-read.js,
 *    planFromReading) instead of walls the reader traced for itself, in the
 *    wall's ink band, cut at piers, up to 20ft. A v15 record on Plan A had
 *    three front windows missing and a 10ft "interior window" over a closet.
 * 17: the openings' bands come from the ink at the jambs and the lines across
 *    the gap, a bay's glass is read at its side walls' tips, corner doorways
 *    and bay fronts are sealed before the flood, the flood runs over the
 *    walls rather than the ink, and one window is never two. A v16 record on
 *    The Sky had a 71px-deep "window" at a bay's mouth and four deck windows
 *    missing.
 * 18: an interior window needs a glazing line between the faces (innerLine):
 *    a wall drawn hollow between two solid junctions is no longer one.
 * 19: the openings are read between the walls of the trace the record SHIPS
 *    (readPlan in window-read.js), not the trace at the assumed width that
 *    only chose the render. A v18 record on The Star carried a 12.9ft
 *    "window" down a bedroom's west wall, read off a closet corner the 40ft
 *    trace had sealed into one block; the walls it shipped had no such gap.
 * 20: the record carries DOORS -- every doorway the door finder hung a leaf
 *    in (doors.js, attachDoors), with the hinge, the leaf's length and the
 *    angle the plan draws it at -- and the viewer stands them up. A v19
 *    record has no doors at all, and a visitor would see every doorway open.
 * 21: the record carries FIREPLACES (fireplace.js): the mouth, the firebox's
 *    back and the hearth, read from the drafter's symbol; and a firebox is
 *    no longer a doorway to the door readers. A v20 record on The Star has
 *    a pair of doors standing in the fireplace.
 * 22: a pocket door travels shut, across its doorway, with the pocket WALL
 *    to build beside it (`wall`): the plan draws that wall hollow and the
 *    tracer leaves it out. A v21 record stood the panel half open in an
 *    opening twice its width.
 * 23: doors are read ARC FIRST as well (doors.js, findArcDoors): in every
 *    doorway the hinge-first finder left bare, the sweep is looked for on
 *    the doorway's own jambs at the doorway's own width, and the leaf where
 *    the arc says it must be -- open at its end, shut along the doorway, or
 *    lying flat against the next wall. Against the door truth this took the
 *    reader from 305 doors to 385 of 451 with none invented. A v22 record
 *    has one door in six missing.
 * 24: doors are read from their SYMBOL first (door-symbols.js): every arc the
 *    drawing carries is found where it is drawn, its hinge put on the wall
 *    by the leaf, and its chord is the doorway -- hung in the traced doorway
 *    it lies along, or, where the model left the wall open but listed no
 *    gap (a corner, a wall's end), made into a doorway of its own. A v23
 *    record has no door where the tracer listed no doorway: Jordan's four,
 *    Madison 4's five, the Sky's three.
 * 25: a CLOSET is read as a thing (closets.js): a doorway with a shallow
 *    closed cell behind it and a rod, a shelf or a row of hangers inside is
 *    a closet's mouth, and only there the bypass pair -- two panels on two
 *    tracks, a sliding window's symbol anywhere else -- is its doors, built
 *    shut, and a single bar across it its door drawn shut. A v24 record has
 *    every bypass closet open.
 * 26: a PAIR is read from its symbol too (door-symbols.js): two half leaves
 *    facing each other on one wall line, their chords meeting in the middle,
 *    make one doorway hinge to hinge where the model listed none (The Sky's
 *    bedroom closets); and a symbol's chord is matched to the model's
 *    doorway with the extruder's wall thickness, so a symbol over a listed
 *    doorway no longer makes a second one on top of it. A v25 record has a
 *    closet pair missing and, on some plans, a doorway twice.
 * 27: ONE DOORWAY IS LISTED ONCE (window-read.js). A wall that jogs at a
 *    doorway had it listed from both lines and the door hung in each, two
 *    leaves through one another (The Sky's hall); a closet's or a kitchen's
 *    side walls ending at a wall were read as a bay's, and its "front" laid
 *    on the doorway or the window in that wall (Madison's pantry, its
 *    kitchen -- an 8ft window inside the room; Run 9 -- glass across a
 *    whole wall). A v26 record has those doors twice and those windows.
 * 28: a decision the reader no longer finds an opening for is kept only
 *    when it was the PERSON'S (window-read.js, applyDecisions): the list
 *    is written whole, reader's verdicts and all, and a verdict on a
 *    doorway the tracer had listed twice outlived the fix and stood as a
 *    shut door beside the open one (The Sky's hall). A v27 record can
 *    carry such a door.
 * 29: the arc reader's ink must have the SHAPE it is taken for (doors.js,
 *    findArcDoors): the arc's pixels bend and fit a circle, the leaf's do
 *    not fit one. Sampling "ink within three pixels" along a circle and a
 *    ray was met by a closet's bifold stroke and the next door's arc, and
 *    Plan A had a door at a cased opening the plan leaves open; a stair's
 *    treads and a bay's trapezoid hung two more. A v28 record can carry
 *    those doors.
 * 30: a leaf is FOOTED on the wall's face (window-read.js, footLeaf): the
 *    hinge slides along the leaf's line to the face of the doorway's band
 *    it rises from, the tip staying put. The symbol reader hung Plan A's
 *    bedroom door half a foot off its wall, foot in the air. A v29 record
 *    has leaves floating off, or half inside, their walls.
 * 31: a leaf hangs from the JAMB (footLeaf): the hinge slides along the wall
 *    to the nearer jamb, the tip staying, so the radius and the angle follow
 *    (Plan A's front door stood in the opening with air on both sides). A
 *    sliding door's panels are read within the wall's reach only, and the
 *    reader's own gap counts as the doorway only in the same wall band
 *    (Plan A's closet doors were built inside the far wall). A fold whose
 *    two strokes are crossed by one line at one height is a box bay's
 *    trapezoid, not a bifold; one fold with no arc from its jambs is a
 *    two-panel bifold, with arcs a pair. A v30 record can carry those.
 * 32: the footing's bound is across the wall (a leaf half open was left
 *    unfooted by a bound along it), a hinge on the wall's side of the face
 *    is footed whenever the band is a wall's, and the jamb is taken whether
 *    or not the face was (footLeaf). The gate counts leaves off their jamb;
 *    three on the corpus went to theirs. A v31 record can carry them.
 * 33: a symbol hangs in the doorway whose band its hinge is nearest, never
 *    one wider than a pair (gapOf): an 11ft hall gap 49px off took a 3ft
 *    doorway's symbol by overlap and then hung nothing (apt413201 #14);
 *    Madison 2 and 3 each gain a door made from a symbol the wide gap had
 *    swallowed. A v32 record can carry them.
 * 34: the hinge-first reader (findDoors) holds its candidates to the shape
 *    tests the arc reader has had since v29 (shapeTests): the corpus's one
 *    invented door (apt413201, a basin's edge and oval) is gone, and with it
 *    a stair's stringer and a closet's top edge that had passed as doors
 *    while the truth agreed. staysOn's ends are a length, not a share of
 *    the samples, so a half-open pair's second arc is not "ink leaving" the
 *    first. Jordan 4's wedge-drawn exterior door, no arc, is a miss now.
 *    A v33 record can carry them.
 * 35: THE DOORS ARE A SCALE WITNESS (readPlan): where no label calibrates
 *    the width, the median hung single leaf is read as a 2'-8" door and the
 *    plan traced again at the width that makes it so, when that moves the
 *    width by more than the witness's own tenth. The corpus stood on the
 *    assumed 40ft on 33 of 36 renders; 20 of them move, 31 to 56ft. With the
 *    scale right, two bands measured at 40ft moved with it: a pair's leaf
 *    is 18in (MIN_HALF_FT), and a glazed opening is a window to 9ft. Two
 *    more did not, and are the next work: the extruder's brush closes The
 *    Balcony's bathroom window at 31.6ft; another2's bifold reads as one
 *    Lambda, not two, at 50.9ft. A v34 record can carry them.
 */
// 36 (2026-09-21): the record carries `gaps` beside `openings` (cloud-publish
// readingOf), so a visitor's page can flood the sheet and tell a porch from
// a room for the stairs (stairs.js outsideOf). A record without them still
// draws; its stairs fall back to the nearest label's name.
// v37 (2026-09-24): a deck's, porch's or bay's hatch drawn light enough to
// pass the wall threshold is fill, not wall (fillRegions); on Jordan 4 its
// decks and bays had stood as solid blocks. Every other render's mask is
// unchanged pixel for pixel.
// v38 (2026-09-24): a wall thinner than the plan's wall floor is simplified
// at its own thickness (outline.js wallShapes), not the floor's; a thin
// partition's free end had lost its corners and the wall tapered along its
// length, worse the thicker OTHER walls were. Four renders of 36 move, each
// covering more of its ink.
// v39 (2026-09-24): the symbols are read on their own ink. A wall stub that
// grows out of a wall end-on is a wall (isWallStub; a closet front's returns
// had gone as dust). A window's hairlines, a closet's rod and shelf and its
// doors' panels are read on the symbols' ink (window-read.js symbolInk: the
// anti-aliased hairline the fixed cut broke, along an axis, never a tone),
// a window's frame may lie half a wall past its faces, a closet's rod is
// looked for on every row and its doors past 5ft, and a panel drawn from one
// jamb is its door. Jordan 4 gains its suite and flex room windows and its
// bedroom closet's doors; the doors gate holds at 87.3%.
// v40 (2026-09-24): a pair of doors drawn nearly shut (findShutPairs): two
// leaves a few degrees open, each turning square back to the door line, the
// turns meeting at the middle -- their arcs too short to read as arcs -- and
// where the model lists no doorway under them, the pair makes one (Plan A's
// linen closet, whose mouth lies between the tips of two parallel walls).
// v41 (2026-09-24): a closet drawn in thin lines is built (findThinClosets):
// a rod with hangers, the box of hairlines around it, its thin sides built
// at least as thick as the outlines carry (thinnestTraced) and never cut by
// a doorway beside them, a lining filled back to the wall behind it, its
// mouth listed as a doorway, and a pair drawn nearly shut read before a bar
// across a closet's mouth (Plan A's closet beside BATH 2). And a single
// leaf hangs in a gap whose rest is too short for another leaf (MIN_R_FT):
// Plan A's master bath door no longer hangs on the arc fit's draw.
// v42 (2026-09-24): a closet's hangers are a row -- one tick at one pitch
// (hangerRun, dashedRun's rule) -- and furniture's outlines crossing a row
// are not: Run 9's bath read as a closet from its basin, and Sky 2b's double
// vanity as a closet with a 9ft door.
// v43 (2026-09-24): a gap's band is read where its wall runs on past any wall
// standing across a jamb (bandFromInk), so a bay's mouth and a hall's end no
// longer take the cross wall's length for their thickness; and a pier is
// drawn as strong as the wall beside it (splitAtPiers), so a hatch drawn
// through a doorway does not cut it, and a window's frame line no longer
// starts the window inside the wall's end.
// v44 (2026-09-24): the doors' witness to the width is taken by degrees
// (witnessShare): none within its tenth, all of it past twice that, a share
// that grows in between. Nine renders read at a new scale; a render whose
// doors are just past a tenth off no longer moves by the whole tenth.
// v45 (2026-09-24): the symbol reader's hinge search gives a tie to the
// arc's own fit, not to the first position scanned (door-symbols.js): a leaf
// drawn as two lines passed its test over a range of hinges, and The Star's
// hall closet was hinged off its wall and refused. Three doors more (The
// Star, Another, Another 2); five made doorways land on their wall line.
// v46 (2026-09-25): a garage door is an exterior opening of the garage at
// least as wide as the car the model parks (CATALOG.car), found whatever is
// drawn across it -- dark lines, two hairlines, nothing -- as long as the
// wall is open there (jambWallTone); a narrower opening keeps what the
// drawing says, where a garage's window or side door had become a garage
// door. "GAR" names a garage and a carport has no vehicle door (garage.js).
// And a slice's end is not a wall end (endGaps): a wall thickened at its end
// no longer lists an "opening" along the floor beside it -- 20 listings on
// 9 renders, Jordan 2's 19ft garage wall among them and one window Jordan
// had framed on a solid wall.
// v47 (2026-09-25): one doorway, one entry. A doorway with a line along it
// (a threshold) was listed twice, as a bare opening and as the door hung in
// it, because the exterior list held copies of the gaps and was asked by
// object: 16 on 13 renders, every one a door. Named "Door" in the panel,
// the copy stood a shut door beside the open one. Listed and drawn by the
// rectangle now; Another 2's bypass door is no longer also a window; and a
// saved decision's items over one opening are all spent on it
// (applyDecisions), the person's choice first.
export const GEOMETRY_VERSION = 47;
/**
 * The oldest published reading a visitor's page draws as it is. Version 15
 * is where a record began to carry `openings` with their kinds and units
 * (the market look) and the `inkRects` the outlines are cut around; before
 * it a record has holes where the windows are. Raise this only when a field
 * the engine cannot do without is added -- and say so here.
 */
export const MIN_SHIPPED_VERSION = 15;

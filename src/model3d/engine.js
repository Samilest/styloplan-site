...
import {
  wallPieces, extentOf, DOOR_H, WALL_T, subtractRects, windowPieces, HEAD, marketSill, leafPose, LEAF_T, FIRE,
} from './geometry.js';
...
import { stairBoxes, cutWells } from './stairs.js';
 ...
 */
/**
 * A NUMBERED PIN over an opening, for the builder's Windows panel: the same
 * disc the Review checklist pins its rooms with (an accent-soft circle with
 * the number in accent), standing on the wall's top so it can be matched to
 * its row. Sized on screen every frame like a label (layoutMarkers in
 * view3d.html); `hot` is the pin whose row the pointer is on, drawn in the
 * selection colour and a little larger.
 */
function markerSprite(n, atX, atZ, y, colours) {
  const cv = document.createElement('canvas');
  cv.width = 160; cv.height = 160;
  const ctx = cv.getContext('2d');
  const draw = (fill, ink, ring) => {
    ctx.clearRect(0, 0, 160, 160);
    ctx.beginPath(); ctx.arc(80, 80, 66, 0, Math.PI * 2);
    ctx.fillStyle = fill; ctx.fill();
    ctx.lineWidth = 10; ctx.strokeStyle = ring; ctx.stroke();
    ctx.fillStyle = ink; ctx.font = '500 84px Roboto, Inter, system-ui, sans-serif';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(String(n), 80, 84);
  };
  draw(colours.fill, colours.ink, colours.ring);
  const tex = new THREE.CanvasTexture(cv);
  tex.anisotropy = 4;
  const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, transparent: true, depthTest: false }));
  sp.position.set(atX, y, atZ);
  sp.userData = { marker: n, aspect: 1, hot: false, redraw: (hot) => {
    if (hot) draw(colours.hotFill, colours.hotInk, colours.hotRing); else draw(colours.fill, colours.ink, colours.ring);
 ...
 * @param {Array}  [opts.market]  openings to frame as windows, shutter as garage
 *   doors, or hang a door's leaves in (`kind`, `units`, `leaves`), in model
 *   units; see the block below
 * @param {boolean} [opts.belowGrade]  the floor is a basement: market windows
 *   keep the small high light instead of the tall one (level.js decides)
 *
 * STAIRS STAND SINCE 2026-09-21, through `opts.stairs3d`: the reading that
 * src/model3d/stairs.js makes of the floor's confirmed stairs on its render
 * (readStairs). A DN stair is a WELL cut into the floor texture with its
 * treads descending inside, an UP stair rises from the floor, and the flights
 * of one staircase are one stair, ordered along travel. This builder turns
 * the reading's boxes into meshes and cuts its wells out of a copy of the
 * floor; it decides nothing about the stairs itself. A reading that is `flat`
 * (a confirmed stair it could not read) builds nothing -- ONE STAIR RAISED AND
 * TWO LEFT FLAT ON THE SAME FLOOR IS WORSE THAN THREE FLAT; a customer reads
 * the inconsistency as a fault, and it is one. The first attempt at this
 * (2026-09-03, removed) matched one stair in three; stairs.js says what was
 * wrong with it and what was measured since. The 2D label sprite (UP/DN)
 * below is unchanged and still comes from the record alone.
 *
...
export function buildExtruded(ex, opts = {}) {
  ...
  // THE PLAN, WALLS AND ALL, laid over the model for the flat view: the same
  // crop of the render with nothing painted out (extrude.js, planFloor), a
  // hair above the floor so it hides the walls once they are down, and NOT
  // in the group -- the group's y is squashed as the walls sink, and this
  // plane must not sink with them. Built on first use; view3d adds it to the
  // scene beside the group and fades it in as the walls go down.
  let planFloor = () => null;
  // THE STAIRS' BOXES AND WELLS, from the reading (stairs.js). The wells are
  // cut out of a COPY of the sheet -- `ex.floor` is shared by every build of
  // this floor, and a rebuild for the other look must start from the whole
  // sheet -- and the plane that carries it must not write depth where it is
  // clear, or the treads below floor level stay hidden behind it.
  const stair = stairBoxes(opts.stairs3d);
  if (ex.floor) {
    const sheet = stair.wells.length && ex.floorRect ? cutWells(ex.floor, stair.wells, ex.floorRect) : ex.floor;
    ...
    // The sheet's two regions, as alpha masks from the one occluder (its
    // green channel is what three reads): the surface where it is white, the
    // paper where it is black, a well cut from both as it is from the sheet.
    // Complementary at one threshold, so no pixel is both or neither.
    const maskOf = (invert) => {
      const src = stair.wells.length && ex.floorRect ? cutWells(ex.floor.occluder, stair.wells, ex.floorRect) : ex.floor.occluder;
      let c = src;
      if (invert) {
        ...
        // A cleared well stays clear (alpha 0 -> nothing), the rest inverts.
        for (let i = 0; i < a.length; i += 4) { if (a[i + 3] === 0) continue; a[i] = a[i + 1] = a[i + 2] = 255 - a[i + 1]; }
    ...
    const surfaceMask = () => maskOf(false), paperMask = () => maskOf(true);
    ...
    const floorMat = keep(new THREE.MeshBasicMaterial({
      ...
      transparent: true,
      // A cleared pixel (a stair well, or the surface region below) is
      // discarded outright; the sheet's own fade is far above this.
      ...(stair.wells.length || ex.floor.occluder ? { alphaTest: 0.02 } : {}),
      // PAPER IS NOT A SURFACE. A porch's steps descend below the sheet, and
      // from any oblique view the paper's rim beside them lay in front of the
      // lower treads -- lifted paper, so it read as a smear of shadow on the
      // steps (Saman, 2026-09-21, both looks). When the sheet carries an
      // occluder (extrude.js: white where it is a surface, black where it is
      // lifted paper), this plane is the PAPER alone -- the occluder's
      // inverse as its alpha, so it is discarded wherever the sheet is a
      // surface -- and writes no depth; the surface is a second plane below,
      // opaque. The deck's and the floor's rims still hide what lies under
      // them, the paper's does not, and an exterior stair's treads (drawn
      // after this pass, below) show through the paper alone.
      //
      // TWO REGIONS, NOT TWO PASSES. The first version drew the colour of the
      // whole sheet here and its depth from a second, unseen mesh, and the two
      // did not cover the same pixels: at a well's edge the depth twin (a
      // quarter-size mask, one threshold) reached a pixel or two past the
      // colour's cut (full size, another threshold), rejected the treads
      // below and drew nothing -- a line of background along every well and
      // wall base (Saman, same day, both looks). A fragment must decide its
      // colour and its depth TOGETHER, so each region is one material.
      ...(ex.floor.occluder ? { alphaMap: paperMask(), depthWrite: false } : {}),
    }));
    const plane = new THREE.Mesh(geo, floorMat);
    plane.rotation.x = -Math.PI / 2;
    plane.position.set(fr.x0 + fw / 2, 0, fr.z0 + fd / 2);
    // THE GROUND IS DRAWN FIRST AMONG THE SEE-THROUGH THINGS, and this is a
    // fix, not a preference. Being transparent (for the sheet's edge fade)
    // puts the ground in three's transparent pass, which draws objects far to
    // near by their POSITION -- this plane's centre against the merged panes'
    // origin -- and the winner flipped with the camera: from outside The
    // Sky's deck wall the ground came after the glass and painted over it
    // wherever the room's floor lay behind a pane (the panes do not write
    // depth), so every window read as an open hole; from inside, the other
    // order, and the glass was there. Saman, 2026-09-20: "it shows the
    // glass when I turn it". The ground is under everything and never in
    // front of anything, so it goes first, whatever the angle.
    plane.renderOrder = -1;
    group.add(plane);
    if (ex.floor.occluder) {
      // THE SURFACE: the same sheet, the occluder as its alpha, opaque and
      // writing depth like a wall. Drawn in the opaque pass, so it hides
      // what lies under the deck's and the floor's rims (a stair's lower
      // treads seen through the deck) and is hidden by nothing it should
      // not be. A well is cut from the occluder as it is from the sheet.
      const surfMat = keep(new THREE.MeshBasicMaterial({
        map: tex, alphaMap: surfaceMask(), alphaTest: 0.5,
        polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1,
      }));
      const surf = new THREE.Mesh(geo, surfMat);
      surf.rotation.x = -Math.PI / 2;
      surf.position.copy(plane.position);
      group.add(surf);
    }
    let planMesh = null;
    planFloor = () => {
      if (planMesh || typeof ex.planFloor !== 'function') return planMesh;
      const canvas = ex.planFloor();
      if (!canvas) return null;
      const t2 = keep(new THREE.CanvasTexture(canvas));
      t2.colorSpace = THREE.SRGBColorSpace;
      t2.minFilter = THREE.LinearMipmapLinearFilter;
      t2.generateMipmaps = true;
      t2.anisotropy = 8;
      const m2 = keep(new THREE.MeshBasicMaterial({ map: t2, transparent: true, opacity: 0, depthWrite: false }));
      planMesh = new THREE.Mesh(geo, m2);
      planMesh.rotation.x = -Math.PI / 2;
      // Above the tallest a sunk wall gets (SECTION_H at the flat view's floor
      // scale) and below anything a camera can tell apart from the ground.
      planMesh.position.set(fr.x0 + fw / 2, 0.06, fr.z0 + fd / 2);
  ...
  // A WALL THE PLAN DREW AS A DOOR'S LEAF IS NOT A WALL. The Sky draws a
  // leaf as a solid bar half a wall thick, and the tracer builds it -- so a
  // closet's pair stood as two wall stubs with the door leaves hidden inside
  // them, and Saman saw no door (2026-09-14/18). Where a hung leaf lies along
  // a thin, leaf-long wall rectangle, that rectangle is left out and the leaf
  // is what stands there, in the door's tone.
  const leafLines = [];
  for (const m of opts.market || []) {
    if (m.kind !== 'door') continue;
    for (const lf of m.leaves || []) {
      const { cx, cz, dx, dz } = leafPose(lf);
      leafLines.push({ cx, cz, dx, dz, len: lf.len });
    }
  }
  const wallT = ex.wallFloorFt || WALL_T;
  const isLeafBar = (r) => {
    const w = r.x1 - r.x0, d = r.z1 - r.z0;
    const thin = Math.min(w, d), long = Math.max(w, d);
    if (thin > wallT * 0.6 || long > 5 || long < 1) return false;
    const alongX = w >= d;
    const mid = alongX ? (r.z0 + r.z1) / 2 : (r.x0 + r.x1) / 2;
    return leafLines.some((l) => {
      const leafAlongX = Math.abs(l.dx) >= Math.abs(l.dz);
      if (leafAlongX !== alongX) return false;
      const lMid = alongX ? l.cz : l.cx;
      if (Math.abs(lMid - mid) > Math.max(LEAF_T * 3, wallT * 0.5)) return false;
      const l0 = (alongX ? l.cx : l.cz) - l.len / 2, l1 = l0 + l.len;
      const r0 = alongX ? r.x0 : r.z0, r1 = alongX ? r.x1 : r.z1;
      return Math.min(l1, r1) - Math.max(l0, r0) >= long * 0.5;
    });
  };
  const solidWalls = subtractRects(leafLines.length ? ex.walls.filter((r) => !isLeafBar(r)) : ex.walls, frames);
  ...
  // A WINDOW AS A PANE OF THE WALL, when asked for (`opts.glass`).
  //
  // The wall is already continuous through the opening (wallMask painted the
  // window back in as wall, see `fill`). What was missing was any sign that
  // this stretch of wall is glass. Six attempts put geometry IN or OVER the
  // opening and every one was rejected on look: a gap reads as no wall, a
  // notch as a wall standing in the window, a header as a beam over a hole.
  // This changes no silhouette at all. The same full-height box stands where
  // the window is, in the theme's glass tone and slightly transparent, so
  // from above the wall's top runs unbroken with a lighter band where the
  // glazing is -- which is how a section drawing shows glass -- and from the
  // orbit angle the outer face carries the same band.
  //
  // `frames` are the window rectangles already subtracted from the wall boxes
  // above; each gets its box back, in glass.
  if (opts.glass) {
    const glassMat = keep(new THREE.MeshLambertMaterial({
      color: T.glass ?? T.wall, transparent: true, opacity: 0.6,
    }));
    for (const r of frames) {
      const w = r.x1 - r.x0, d = r.z1 - r.z0;
      if (w <= 0 || d <= 0) continue;
      const g = keep(new THREE.BoxGeometry(w, SECTION_H, d));
      const m = new THREE.Mesh(g, glassMat);
      m.position.set((r.x0 + r.x1) / 2, SECTION_H / 2, (r.z0 + r.z1) / 2);
      group.add(m);
      if (showEdges) {
        const edge = new THREE.LineSegments(keep(new THREE.EdgesGeometry(g)), edgeMat);
        edge.position.copy(m.position);
  ...
  // THE MARKET'S WINDOW (`opts.market`): what Floorplanner, RoomSketcher,
  // CubiCasa and BoxBrownie all draw (docs/research-3d-openings-2026-09-12).
  // A sill of wall from the floor to a standard height, two jambs, a head
  // rail at the cut, and between them a translucent pane with a mullion.
  // Seven earlier attempts put a sill alone (a wall standing in the window),
  // a tinted solid (no window), or a beam over a gap; none had a frame and a
  // pane, which is what the eye reads as glass from above -- a thin frame
  // line across the opening with the pane's tint under it. The sill height
  // is a convention, as the market's is (RoomSketcher: "standard height
  // unless changed"); the caption on the stage already says details may vary.
  // Judged on The Star and the default since 2026-09-13; `?windows=open` in
  // view3d is the look before it.
  if (opts.market?.length) {
    // THE SILL IS WHERE THE GROUND IS. Below grade the wall under a window is
    // earth, so it keeps the market's small high light (sill at half the
    // section, the ~3ft sill under a ~6.2ft cut). Above grade the sill drops to
    // a quarter and the pane is half again as tall -- Saman's rule, 2026-09-12.
    // Which it is comes from the caller (`opts.belowGrade`, decided by
    // level.js from the customer's own words); nothing here reads the drawing.
    const SILL = marketSill(!!opts.belowGrade);
    const FRAME = Math.max(0.12, WALL_T * 0.45);
    // THE PANE HAS TO BE SEEN FROM ABOVE, which is where the model is looked
    // at. A vertical sheet at the wall's tone with a third of its opacity was
    // invisible from the default view -- Saman looked and saw no window at
    // all. So the pane is a glass tint that contrasts with the wall in both
    // looks (CubiCasa draws a dark pane in a white frame), thick enough that
    // its top edge reads as a line inside the frame from straight above.
    const glassMat = keep(new THREE.MeshLambertMaterial({
      color: opts.theme === 'light' ? '#4f6f8e' : '#8fb4d6', transparent: true, opacity: 0.7, depthWrite: false,
    }));
    const box = (x0, y0, z0, x1, y1, z1, mat) => {
      ...
      group.add(m);
      if (showEdges && mat !== glassMat) {
        const edge = new THREE.LineSegments(keep(new THREE.EdgesGeometry(g)), edgeMat);
        edge.position.copy(m.position);
        group.add(edge);
      }
    };
    // A GARAGE DOOR IS A ROLLER SHUTTER: a panel the wall's own height across
    // the whole opening, in the leaf tone, with horizontal slat lines on its
    // face. Saman's rule: any exterior opening of a room named GARAGE.
    const leafMat = keep(new THREE.MeshLambertMaterial({ color: T.leaf ?? T.wall }));
    const slatMat = keep(new THREE.MeshLambertMaterial({ color: T.wall }));
    const doorMat = keep(new THREE.MeshLambertMaterial({ color: T.door ?? T.leaf ?? T.wall }));
    // THE FRAME IS AS THICK AS THE WALL IT SITS IN, not as the band it was
    // read from. The opening's band is measured in the render's ink, which is
    // the wall's full drawn thickness; the walls standing here come from the
    // tracer, which erodes the ink and traces the rects a little thinner. The
    // difference was a sill and two jambs standing proud of both wall faces
    // by a hair -- Saman marked it. So each opening takes the cross section of
    // the wall pieces it interrupts, when they can be found beside it.
    const wallBandAt = (r, along) => {
      const c = along ? [r.z0, r.z1] : [r.x0, r.x1];
      const a = along ? [r.x0, r.x1] : [r.z0, r.z1];
      const beside = ex.walls.filter((w) => {
        const wc = along ? [w.z0, w.z1] : [w.x0, w.x1];
        const wa = along ? [w.x0, w.x1] : [w.z0, w.z1];
        if (Math.min(c[1], wc[1]) - Math.max(c[0], wc[0]) <= 0) return false;   // not this wall's line
        return Math.abs(wa[1] - a[0]) < 0.6 || Math.abs(wa[0] - a[1]) < 0.6;    // meeting an end
      });
      if (!beside.length) return c;
      // A wall arrives from the tracer as several parallel strips more often
      // than as one rect (the bath's wall here: five pieces across 0.63),
      // so it is their union that is the wall -- never wider than the band
      // the opening was read in.
      const lo = Math.min(...beside.map((w) => (along ? w.z0 : w.x0)));
      const hi = Math.max(...beside.map((w) => (along ? w.z1 : w.x1)));
      return [Math.max(lo, c[0]), Math.min(hi, c[1])];
    };
    for (const m of opts.market) {
      const along = m.horizontal ?? (m.x1 - m.x0 >= m.z1 - m.z0);
      const band = wallBandAt(m, along);
      const r = along ? { ...m, z0: band[0], z1: band[1] } : { ...m, x0: band[0], x1: band[1] };
      const mat = isEnvelope(r) ? wallMat : innerMat;
      if (r.kind === 'garage') {
        const cx = (r.x0 + r.x1) / 2, cz = (r.z0 + r.z1) / 2;
        const t = Math.max(0.1, (along ? r.z1 - r.z0 : r.x1 - r.x0) * 0.5);
        if (along) box(r.x0, 0, cz - t / 2, r.x1, SECTION_H, cz + t / 2, leafMat);
        else box(cx - t / 2, 0, r.z0, cx + t / 2, SECTION_H, r.z1, leafMat);
        // Slats: a thin line every foot and a half, proud of the panel by a hair
        // on both faces so it reads from either side.
        for (let y = 1.5; y < SECTION_H - 0.5; y += 1.5) {
          if (along) box(r.x0, y - 0.03, cz - t / 2 - 0.02, r.x1, y + 0.03, cz + t / 2 + 0.02, slatMat);
          else box(cx - t / 2 - 0.02, y - 0.03, r.z0, cx + t / 2 + 0.02, y + 0.03, r.z1, slatMat);
        }
        continue;
      }
      // A DOOR IS ITS LEAF, STOOD UP WHERE THE PLAN DRAWS IT. The reader hands
      // over the hinge, the leaf's length and the angle the drafter drew it
      // open at (attachDoors in window-read.js); the leaf stands on that
      // line, floor to the cut, in the door tone (theme3d: the wall's solid
      // halfway to the floor) and WITHOUT the wall's edge lines -- with them,
      // and in the wall's own tone, a leaf read as a stub of wall at every
      // doorway. From above it sits exactly on its own symbol; from the orbit
      // angle the doorway reads as a door with its swing, the way every
      // dollhouse view in the market draws one. No frame: the jambs are the
      // wall ends themselves, and the head is above the cut like every
      // window's. A pair of French doors is two leaves from two hinges. A
      // door the person named where the reader hung no leaf (asKind) has no
      // hinge to stand on and is drawn SHUT: a thin panel across the opening,
      // which is honest and still a door.
      // A FIREPLACE, AS THE DRAFTER'S BLOCK DRAWS IT (fireplace.js, FIRE in
      // geometry.js): the breast's walls already stand; across the mouth a
      // lintel from the opening's head to the cut; behind it the firebox --
      // its floor, its two splayed sides and its back, soot-dark, to the
      // opening's height; on the room side a surround of two pilasters and
      // a shelf, and the hearth as a slab a hand high over the rectangle the
      // plan draws. `r.fire` is in the model's feet, mapped by the caller.
      if (r.kind === 'fireplace' && r.fire) {
        const F = r.fire;
        const a0 = along ? r.x0 : r.z0, a1 = along ? r.x1 : r.z1;
        const c0 = along ? r.z0 : r.x0, c1 = along ? r.z1 : r.x1;
        const room = F.side > 0 ? c0 : c1;       // the band's face towards the room
        const inner = F.side > 0 ? c1 : c0;      // the face the firebox opens from
        const out = -F.side;                     // towards the room
        const pl = (p0, y0, q0, p1, y1, q1, m) => (along
          ? box(Math.min(p0, p1), y0, Math.min(q0, q1), Math.max(p0, p1), y1, Math.max(q0, q1), m)
          : box(Math.min(q0, q1), y0, Math.min(p0, p1), Math.max(q0, q1), y1, Math.max(p0, p1), m));
        // The lintel: the breast's face above the opening, the band's own thickness.
        pl(a0, FIRE.open, c0, a1, SECTION_H, c1, mat);
        // The firebox: floor, back and the two splayed sides, up to the opening's head.
        const bc = F.backC;
        const dark = doorMat;
        pl(a0, 0, inner, a1, 0.04, bc, dark);
        pl(F.back[0], 0, bc - F.side * 0.05, F.back[1], FIRE.open, bc + F.side * 0.05, dark);
        for (const [pa, pb] of [[a0, F.back[0]], [a1, F.back[1]]]) {
          const [x0, z0] = along ? [pa, inner] : [inner, pa];
          const [x1, z1] = along ? [pb, bc] : [bc, pb];
          const len = Math.hypot(x1 - x0, z1 - z0);
          if (len < 0.05) continue;
          const g = keep(new THREE.BoxGeometry(len, FIRE.open, LEAF_T));
          const m = new THREE.Mesh(g, dark);
          m.position.set((x0 + x1) / 2, FIRE.open / 2, (z0 + z1) / 2);
          m.rotation.y = Math.atan2(-(z1 - z0), x1 - x0);
          group.add(m);
        }
        // The surround: two pilasters beside the opening and a shelf over it, proud of the face.
        const face = room, proud = room + out * FIRE.proud;
        pl(a0 - FIRE.pilaster - 0.1, 0, face, a0 - 0.1, FIRE.shelf, proud, mat);
        pl(a1 + 0.1, 0, face, a1 + FIRE.pilaster + 0.1, FIRE.shelf, proud, mat);
        pl(a0 - FIRE.pilaster - 0.25, FIRE.shelf, face, a1 + FIRE.pilaster + 0.25, FIRE.shelf + FIRE.shelfT, room + out * FIRE.shelfOut, mat);
        // The hearth, where the plan draws one.
        if (F.hearth) pl(F.hearth.lo, 0, face, F.hearth.hi, FIRE.hearthH, F.hearth.far, leafMat);
        continue;
      }
      if (r.kind === 'door') {
        // A POCKET DOOR'S WALL, built here because the tracer left it out:
        // the plan draws the wall hollow where the panel slides into it, and
        // a hollow wall is no wall to the brush. Without it the door stood
        // in an opening twice its width (The Sky's walk-in closet). Wall
        // tone, full height, the band's own thickness.
        if (r.wall) {
          const w = r.wall;
          box(Math.min(w.x0, w.x1), 0, Math.min(w.z0, w.z1), Math.max(w.x0, w.x1), SECTION_H, Math.max(w.z0, w.z1), mat);
        }
        if (r.leaves?.length) {
          for (const lf of r.leaves) {
            let { cx, cz, yaw, dx, dz } = leafPose(lf);
            // A LEAF DRAWN FLAT AGAINST A WALL STANDS PROUD OF IT. The plan
            // draws a door opened a full quarter with its leaf on the next
            // wall's face; read there, the leaf's centre line lands inside
            // the wall's box and the panel vanishes into it (The Sky, bedroom
            // 1: the closet's left leaf against the outer wall, the right one
            // plain to see). Pushed out to the face by its own thickness.
            const inside = (lf.deg ?? 90) >= 75 ? solidWalls.find((w) => cx > w.x0 && cx < w.x1 && cz > w.z0 && cz < w.z1) : null;
            if (inside) {
              // Out on the DOORWAY'S side, never the nearer one: the swept
              // sector lies between the shut line (`ax, az`, from the hinge
              // toward the far jamb) and the leaf, so that is the side the
              // door is on. The nearer face put The Star's hall door on the
              // far side of its wall, over the stairs (2026-09-18).
              const sx = Math.sign(lf.ax || 0), sz = Math.sign(lf.az || 0);
              const d = sx ? (sx > 0 ? inside.x1 - cx : cx - inside.x0) : (sz > 0 ? inside.z1 - cz : cz - inside.z0);
              if (d <= WALL_T) { cx += sx * (d + LEAF_T / 2 + 0.02); cz += sz * (d + LEAF_T / 2 + 0.02); }
            }
            const g = keep(new THREE.BoxGeometry(lf.len, SECTION_H, LEAF_T));
            const m = new THREE.Mesh(g, doorMat);
            m.position.set(cx, SECTION_H / 2, cz);
            m.rotation.y = yaw;
            group.add(m);
          }
        } else {
          const cx = (r.x0 + r.x1) / 2, cz = (r.z0 + r.z1) / 2;
          const g = keep(along
            ? new THREE.BoxGeometry(r.x1 - r.x0, SECTION_H, LEAF_T)
            : new THREE.BoxGeometry(LEAF_T, SECTION_H, r.z1 - r.z0));
          const m = new THREE.Mesh(g, doorMat);
          m.position.set(cx, SECTION_H / 2, cz);
          group.add(m);
        }
      ...
      // Jambs and head rail, in the same tone as the wall. (The sill is
      // drawn per unit below: a patio door has none.)
      if (along) {
        box(r.x0, SILL, r.z0, r.x0 + FRAME, SECTION_H, r.z1, mat);
        box(r.x1 - FRAME, SILL, r.z0, r.x1, SECTION_H, r.z1, mat);
        box(r.x0, SECTION_H - FRAME, r.z0, r.x1, SECTION_H, r.z1, mat);
      } else {
        box(r.x0, SILL, r.z0, r.x1, SECTION_H, r.z0 + FRAME, mat);
        box(r.x0, SILL, r.z1 - FRAME, r.x1, SECTION_H, r.z1, mat);
        box(r.x0, SECTION_H - FRAME, r.z0, r.x1, SECTION_H, r.z1, mat);
      }
      // THE GLASS, BY WHAT THE SYMBOL SAYS. The plan draws its windows three
      // ways (windowStyle in openings.js, read from the drafter's own blocks
      // and confirmed on The Star): a FIXED unit is one pane; a SLIDER is two
      // sashes on two tracks, each a little over half the width, one behind
      // the other in the wall's depth (a PATIO DOOR is the same with two to
      // four panels and no sill); a DOUBLE-HUNG is two sashes stacked in
      // height, the upper on the outer track, with a meeting rail between; a
      // JALOUSIE is a pane behind a stack of slats. A mullion stands where one
      // unit meets the next. Saman looked at the
      // dining room's slider drawn as a fixed pane and said the type differs;
      // it does, and so does what a buyer sees from the orbit angle.
      //
      // Everything is expressed along the run (`a`) and across the wall (`c`)
      // and placed by `place`, so one drawing serves both wall directions.
      const a0 = along ? r.x0 : r.z0, a1 = along ? r.x1 : r.z1;
      const c0 = along ? r.z0 : r.x0, c1 = along ? r.z1 : r.x1;
      const cmid = (c0 + c1) / 2, cth = c1 - c0, t = Math.max(0.08, cth * 0.5);
      const place = (p0, y0, q0, p1, y1, q1, m) => (along ? box(p0, y0, q0, p1, y1, q1, m) : box(q0, y0, p0, q1, y1, p1, m));
      const HEAD_RAIL = SECTION_H - FRAME;
      const units = r.units?.length ? r.units : [{ lo: 0, hi: 1, style: 'fixed', sashes: [] }];
      const span = a1 - a0;
      units.forEach((u, i) => {
        // The unit's own run, less the jambs at the opening's ends and half a
        // mullion where it meets a neighbour.
        const u0 = a0 + u.lo * span + (i === 0 ? FRAME : FRAME / 2);
        const u1 = a0 + u.hi * span - (i === units.length - 1 ? FRAME : FRAME / 2);
        const ua0 = a0 + u.lo * span, ua1 = a0 + u.hi * span;
        // A PATIO DOOR stands on the floor: no sill, glass from the ground.
        // Everything else has the wall's sill under it.
        const base = u.style === 'patio' ? 0 : SILL;
        if (base > 0) place(ua0, 0, c0, ua1, base, c1, mat);
        if (i > 0) place(ua0 - FRAME / 2, 0, c0, ua0 + FRAME / 2, SECTION_H, c1, mat);
        if (u1 - u0 <= 0.05) return;
        if ((u.style === 'slider' || u.style === 'patio') && u.sashes?.length >= 2) {
          // Sashes on alternating tracks, each with its own thin stiles,
          // meeting where they overlap. No mullion. A two-panel unit puts the
          // first inside and the second out; a four-panel patio door puts the
          // outer pair on the outer track and the middle pair inside.
          const w = u1 - u0, stile = FRAME * 0.5, n = u.sashes.length;
          u.sashes.forEach(([lo, hi], k) => {
            const s0 = u0 + lo * w, s1 = u0 + hi * w;
            const outer = n >= 3 ? (k === 0 || k === n - 1) : k === 1;
            const off = (outer ? 1 : -1) * cth * 0.22;
            place(s0, base, cmid + off - t / 4, s1, HEAD_RAIL, cmid + off + t / 4, glassMat);
            place(s0, base, cmid + off - t / 3, s0 + stile, HEAD_RAIL, cmid + off + t / 3, mat);
            place(s1 - stile, base, cmid + off - t / 3, s1, HEAD_RAIL, cmid + off + t / 3, mat);
          });
        } else if (u.style === 'double-hung') {
          // Two sashes stacked: the lower on the inner track, the upper on the
          // outer, a meeting rail where they overlap.
          const mid = SILL + (HEAD_RAIL - SILL) * 0.5, off = cth * 0.18;
          place(u0, SILL, cmid - off - t / 4, u1, mid, cmid - off + t / 4, glassMat);
          place(u0, mid, cmid + off - t / 4, u1, HEAD_RAIL, cmid + off + t / 4, glassMat);
          place(u0, mid - FRAME / 2, c0 + cth * 0.15, u1, mid + FRAME / 2, c1 - cth * 0.15, mat);
        } else if (u.style === 'jalousie') {
          // Louvres: the pane, and a slat across it every few inches, proud
          // of the glass on both faces so the horizontal rhythm reads from
          // either side -- the block's front view is a stack of slats.
          place(u0, SILL, cmid - t / 2, u1, HEAD_RAIL, cmid + t / 2, glassMat);
          const pitch = Math.max(0.25, (HEAD_RAIL - SILL) / 8);
          for (let y = SILL + pitch; y < HEAD_RAIL - pitch * 0.5; y += pitch) {
            place(u0, y - FRAME * 0.2, cmid - t * 0.6, u1, y + FRAME * 0.2, cmid + t * 0.6, mat);
          }
        } else {
          place(u0, SILL, cmid - t / 2, u1, HEAD_RAIL, cmid + t / 2, glassMat);
        }
  ...
  // A HEADER OVER EVERY CUT OPENING, when asked for (`opts.headers`).
  //
  // Five attempts put something INSIDE a window opening and all five were
  // rejected on look; see docs/research-brief-3d-windows-2.md. Every one of
  // them was solving "make the window visible from above". The complaint that
  // brought this here is the opposite one: with the opening cut floor to top,
  // a wall that is mostly windows reads as no wall at all -- on The Star, 86ft
  // of exterior wall, 39% of the kitchen run.
  //
  // So this puts nothing in the opening. It puts wall ABOVE it: a band from the
  // head height to the cut plane, with the opening open beneath. From above the
  // silhouette is continuous, which is what "the wall is there" means; from the
  // orbit angle the opening shows under the band, which is what "there is an
  // opening" means. And it needs no window-versus-door decision, which is where
  // detection is weakest: a patio door read as a window still has a header,
  // and still stays open below it. The only number used is HEAD, which the
  // record path already builds with, and it is a convention, not a measurement.
  //
  // Behind a flag until Saman has looked at it. A sixth attempt on look is his
  // call, not a measurement's.
  if (opts.headers) {
    const band = SECTION_H - HEAD;
    for (const r of frames) {
      const w = r.x1 - r.x0, d = r.z1 - r.z0;
      if (w <= 0 || d <= 0 || band <= 0) continue;
      const g = keep(new THREE.BoxGeometry(w, band, d));
      const m = new THREE.Mesh(g, isEnvelope(r) ? wallMat : innerMat);
      m.position.set((r.x0 + r.x1) / 2, HEAD + band / 2, (r.z0 + r.z1) / 2);
      group.add(m);
      if (showEdges) {
        const edge = new THREE.LineSegments(keep(new THREE.EdgesGeometry(g)), edgeMat);
        edge.position.copy(m.position);
  ...
  // ---- stair direction, from the confirmed record too
  //
  // THE ARROW IS ALREADY HERE AND THE WORD IS NOT. The plan's own arrow
  // survives styling and arrives with the floor picture, but UP and DN are
  // LETTERS, and the styling prompt forbids the model to draw any -- so the 2D
  // composites the word on top and this view drew nothing at all. Saman marked
  // a staircase DN in Review, saw it on the plan, and found it missing on the
  // model.
  //
  // `confirmedStairs` was read here and passed no further, with a comment
  // saying so. That comment was about RAISING a stair block, which is still not
  // done and still right -- the treads come from the drawing. Printing the
  // direction is a different question, and the answer was only ever "nobody
  // wired it".
  //
  // Same sprite, same ink, same mapping through the trim as a room name: it is
  // the same class of thing, a word this layer owns because the image is
  // forbidden to carry it. `printed === false` is honoured exactly as the 2D
  // honours it, so a marker switched off in Review is off in both places.
  // THE STAIRS THEMSELVES: treads, well walls and well floors as boxes in the
  // wall's tone -- dimmed with depth, so a well reads as a hole -- with the
  // same edge lines the walls carry.
  if (stair.boxes.length) {
    const tone = (k) => keep(new THREE.MeshLambertMaterial({ color: new THREE.Color(T.wall).multiplyScalar(k) }));
    const mats = { wellWall: tone(0.55), wellFloor: tone(0.3) };
    const treadMats = new Map();
    const treadMat = (k) => { const key = k.toFixed(2); if (!treadMats.has(key)) treadMats.set(key, tone(Number(key))); return treadMats.get(key); };
    for (const b of stair.boxes) {
      const g = keep(new THREE.BoxGeometry(b.x1 - b.x0, b.y1 - b.y0, b.z1 - b.z0));
      let mat = b.tone === 'tread' ? treadMat(b.k) : mats[b.tone];
      if (b.exterior) {
        // Drawn in the transparent pass after the sheet (renderOrder above
        // the sheet's -1), opaque in every other way; with the sheet not
        // writing depth, the rim of the paper cannot veil these treads.
        mat = keep(mat.clone()); mat.transparent = true; mat.opacity = 1;
      }
      const m = new THREE.Mesh(g, mat);
      if (b.exterior) m.renderOrder = 0;
      m.position.set((b.x0 + b.x1) / 2, (b.y0 + b.y1) / 2, (b.z0 + b.z1) / 2);
      group.add(m);
      if (showEdges) {
        const ed = new THREE.LineSegments(keep(new THREE.EdgesGeometry(g)), edgeMat);
        ed.position.copy(m.position);
        group.add(ed);
      }
    }
    const up = opts.stairs3d.stairs.filter((s) => s.pieces);
    notes.push(`${up.length} stair${up.length === 1 ? '' : 's'} standing: ${up.map((s) => `${s.n} treads ${s.down ? 'down' : 'up'}${s.exterior ? ' (outside, treads only)' : ''}`).join(', ')}.`);
  } else if (opts.stairs3d?.flat) {
    notes.push('Stairs left flat: ' + opts.stairs3d.stairs.filter((s) => !s.pieces).map((s) => s.why.split(' [')[0]).join('; ') + '.');
  }
  // THE POSTS (posts.js, `opts.posts3d`): a porch's or a deck's posts, read
  // off the render as hollow squares outside the walls, stand as the wall
  // does -- its material, its height, its cut cap -- because a post drawn
  // FILLED already stands exactly so, through the tracer (The Avi Top), and
  // the hollow one is the same post in another drafter's pen. Nothing in
  // the plan says how tall a post is, any more than a wall.
  const posts = Array.isArray(opts.posts3d) ? opts.posts3d : [];
  for (const p of posts) {
    const w = p.x1 - p.x0, d = p.z1 - p.z0;
    if (!(w > 0 && d > 0)) continue;
    const g = keep(new THREE.BoxGeometry(w, SECTION_H, d));
    const m = new THREE.Mesh(g, wallMat);
    m.position.set((p.x0 + p.x1) / 2, SECTION_H / 2, (p.z0 + p.z1) / 2);
    group.add(m);
    if (showEdges) {
      const ed = new THREE.LineSegments(keep(new THREE.EdgesGeometry(g)), edgeMat);
      ed.position.copy(m.position);
  ...
  if (posts.length) notes.push(`${posts.length} post${posts.length === 1 ? '' : 's'} standing, read off the render's symbols outside the walls.`);

  for (const st of opts.staircases || []) {
    if (!st?.position || st.printed === false) continue;
    // NO WORD FOR AN UNANSWERED DIRECTION, as the 2D prints none (compositor
    // dirWord). This printed UP for anything that was not 'down', so a stair
    // the reviewer had not answered was labelled UP over steps that went
    // down to the ground (The Star's porch, 2026-09-23).
    if (st.direction !== 'up' && st.direction !== 'down') continue;
    const sx = t ? unTrim(st.position.x, t.x0, t.x1) : st.position.x;
    const sy = t ? unTrim(st.position.y, t.y0, t.y1) : st.position.y;
    labelGroup.add(labelSprite(st.direction === 'down' ? 'DN' : 'UP', null, labelColour,
      e.x0 + sx * W, e.z0 + sy * D, Math.min(W, D) * LABEL_SHARE, labelHalo));
  }
  group.add(labelGroup);

  // THE OPENINGS' PINS (`opts.markers`: `{ n, x, z }` in model feet), for the
  // builder's Windows panel. A separate group: they are not labels (the
  // Labels button leaves them alone) and they are never built for a visitor.
  const markerGroup = new THREE.Group();
  markerGroup.visible = !!opts.markers?.length;
  const pinColours = opts.theme === 'light'
    ? { fill: '#E4EDEE', ink: '#1C5A64', ring: '#FFFFFF', hotFill: '#A8481A', hotInk: '#FFFFFF', hotRing: '#FFFFFF' }
    : { fill: '#1B2E32', ink: '#6FB9C4', ring: '#1E2124', hotFill: '#E08A50', hotInk: '#10262A', hotRing: '#1E2124' };
  for (const m of opts.markers || []) {
    markerGroup.add(markerSprite(m.n, m.x, m.z, SECTION_H + 0.4, pinColours));
  }
  ...
  return {
    ...
    dispose() {
      ...
      markerGroup.children.forEach((sp) => { sp.material.map?.dispose(); sp.material.dispose(); });
      group.clear();
    },
  };
}
...
export function buildFloor(geometry, opts = {}) {
  ...
  if (opts.fixtures !== false) {
    for (const r of geometry.rooms) {
      ...
      // garage.js decides what is a garage, for the image's car count and the
      // model's alike: GAR is one, an apron in front of one is not.
      if (!isGarage(r.label)) continue;

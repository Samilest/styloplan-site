...
/** WCAG-style contrast between two colours, from the palette's own luminance. */
const contrast = (a, b) => {
...
/**
 * The door tone: DARKER than the wall it hangs in, by whichever of two steps
 * stands further from both the wall and the floor. See `door` below.
 *
 * Never lighter. A door lighter than the wall (half the way to white) has
 * the better numbers on a dark palette -- 2.4 to the wall's body -- and on
 * screen it reads as wall: the walls' lit faces and tops are lighter than
 * their body tone, and a light panel in a doorway lands among them (looked
 * at, The Star dark, 2026-09-14). Darker than the body is darker than every
 * face of the wall.
 */
export function doorTone(wall, floor) {
  const candidates = [blend(wall, '#000000', 0.32), blend(wall, floor, 0.5)]
    .filter((c) => luminance(c) < luminance(wall));
  if (!candidates.length) return blend(wall, '#000000', 0.32);
  const apart = (c) => Math.min(contrast(c, wall), contrast(c, floor));
  return candidates.reduce((best, c) => (apart(c) > apart(best) ? c : best));
}
...
export function themeFromPalette(palette) {
  ...
  return {
    ...
    // A garage door reads as a lighter piece of the wall it hangs in, so it is
    // mixed from wall and floor rather than given a colour of its own.
    leaf: hex(blend(palette.walls, palette.floors, 0.5)),
    // A DOOR'S LEAF IS DARKER THAN THE WALL IT HANGS IN, and stands off the
    // floor it swings over. Halfway from the wall to the floor read well on
    // the dark presets and was the wall's own colour on every light one
    // (contrast 1.16-1.22 to the wall; Saman: "the doors are the colour of
    // the walls"). So the step is chosen per palette (doorTone): a third of
    // the way to black, or half the way to the floor, whichever stands
    // further from both surfaces while staying darker than the wall. Over
    // the six presets: to black on every light palette (2.1 to the wall,
    // 2.8 to the floor), to the floor on every dark one (1.5 and 1.5).
    door: hex(doorTone(wallSolid(palette), palette.floors)),

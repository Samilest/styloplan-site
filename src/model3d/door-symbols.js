...
export const MIN_R_FT = 1.4;   // a single leaf; a pair's leaves are half a doorway each
const MAX_R_FT = 5.0;
...
export function findDoorSymbols(mask, W, H, o = {}) {
  ...
  // `salt` moves the draw, and only test/door-luck.mjs sets it: the arcs are
  // RANSAC circles from ONE generator for the whole plan, so what a door
  // reads can hang on the draw, and on every piece read before it. Measured
  // 2026-09-24 with four salts: 237 of 510 door readings changed with the
  // draw (most in hinge or angle, some read or not). See that file.
  const rng = lcg(0x5eed ^ (o.salt || 0));
  ...
  // Read one arc as a door once its shut end is known: the hinge searched
  // on both marks, the leaf found, the sector clear. Returns the record or null.
  const hang = (a, t, seen, shut, open) => {
    const across = shut.d;
    ...
    for (let dy = -reach; dy <= reach; dy += step) for (let dx = -reach; dx <= reach; dx += step) {
      ...
      for (const r of [a.r * 0.85, a.r * 0.92, a.r, a.r * 1.08, a.r * 1.16]) {
        ...
        // A TIE GOES TO THE FIT. Where the leaf is drawn as a double line its
        // test passes over a range of hinges, and the first of them in the
        // scan won: The Star's hall closet was hinged 6px off its wall at 34px
        // for a 40px arc, and the sweep tested from there cut across the leaf
        // ("sector not clear"). The search is for the hinge near the arc's own
        // centre, so among equal scores the nearest to it, and to its radius.
        const off = Math.hypot(dx, dy) + Math.abs(r - a.r);
        if (!best || score > best.score || (score === best.score && off < best.off)) best = { cx, cy, r, arc, leaf, score, againstWall, shut: !againstWall && sh.cover > op.cover, off };
      }
    }
    ...
    if (!best) { seen('no leaf'); return null; }
    if (!sweptIsClear(mask, W, H, [best.cx, best.cy], across, into, best.r, deg, tol)) { seen('sector not clear'); return null; }
    seen('ok');
    return {
  ...
  for (const pts of pcs) {
    for (const raw0 of arcsIn(pts, minR, maxR, Math.max(1.5, lw), rng)) {
      ...
      if (!shut) {
        // A PAIR'S LEAF has no jamb at the far end of its chord -- the other
        // leaf's tip is there. Kept for the pairing below when the chord is
        // otherwise a doorway's (axis-aligned, nothing along it, wall behind).
        const half = ends.filter((e) => e.d && !e.merged && e.thick <= 0.2 && e.along <= 0.5 && e.behind >= 0.5);
        if (half.length) pending.push({ a, t, seen, ends, half });
        else seen('no shut end');
        continue;
      }
      const open = ends.find((e) => e !== shut);
      const d = hang(a, t, seen, shut, open);
      if (d) doors.push(d);
    }
  }
  // PAIRS: two half leaves facing each other on one wall line, the same
  // swing side, their chords meeting in the middle (radii summing to the
  // distance between the hinges within a quarter). Each is read as a door
  // with its chord to the meeting point; the doorway is hinge to hinge.
  for (let i = 0; i < pending.length; i++) {
    ...
    for (let j = i + 1; j < pending.length; j++) {
      ...
      if (Q.done) continue;
      let made = false;
      for (const ep of P.half) for (const eq of Q.half) {
        if (ep.d[0] !== -eq.d[0] || ep.d[1] !== -eq.d[1]) continue;
        const dx = Q.a.cx - P.a.cx, dy = Q.a.cy - P.a.cy;
        const alongD = dx * ep.d[0] + dy * ep.d[1];                 // Q lies in P's shut direction
        const perpD = Math.abs(dx * ep.d[1] - dy * ep.d[0]);
        if (alongD <= 0 || perpD > tol * 2 + lw) continue;
        if (alongD < (P.a.r + Q.a.r) * 0.75 || alongD > (P.a.r + Q.a.r) * 1.3) continue;
        const op = P.ends.find((e) => e !== ep), oq = Q.ends.find((e) => e !== eq);
        const dp = hang(P.a, P.t, P.seen, { ...ep, merged: false }, op);
        const dq = hang(Q.a, Q.t, Q.seen, { ...eq, merged: false }, oq);
        if (!dp || !dq) continue;
        // Both swing the same way.
        if (dp.into[0] !== dq.into[0] || dp.into[1] !== dq.into[1]) continue;
        const mouth = { x0: dp.x, y0: dp.y, x1: dq.x, y1: dq.y };
        dp.pair = mouth; dq.pair = mouth;
        doors.push(dp, dq);
        P.done = Q.done = true; made = true;
        break;
      }
      if (made) break;
    }
    if (!P.done) P.seen('no shut end');
  }

...
/**
 * The sill of a market-look window (engine.js, `opts.market`), as a height.
 *
 * Two heights, by which side of the ground the floor is on (level.js).
 * Below grade the wall under a window is earth, so the window is the small
 * high light the market draws: sill at half the section. Above grade it is the
 * bigger light Saman asked for on 2026-09-12 -- the sill at a quarter, which
 * makes the pane half again as tall. Fractions of the section, not feet: the
 * whole doll's-house cut is a convention and these stay in proportion to it.
 * @param {boolean} below  true for a basement
 * @param {number} [h]     the cut plane; SECTION_H unless a test says otherwise
 */
export const marketSill = (below, h = SECTION_H) => h * (below ? 0.5 : 0.25);

/** A door leaf's thickness, in the model's feet. */
export const LEAF_T = 0.14;

/**
 * THE FIREPLACE'S HEIGHTS, from the drafter's block (Fireplace-mantel.dwg,
 * front views): the opening is about half the mantel's height and two
 * thirds of its width; the shelf sits just above the opening's lintel.
 * Against a 5.0 cut: opening to 2.6, lintel band above it, shelf at 3.4, a
 * hearth a hand high. Conventions, like every height in this model.
 */
export const FIRE = { open: 2.6, shelf: 3.4, shelfT: 0.15, shelfOut: 0.45, pilaster: 0.35, proud: 0.15, hearthH: 0.1 };

/**
 * Where a door's leaf stands: the plan's own symbol, stood up.
 *
 * The reader (doors.js, attachDoors) gives a leaf as its hinge, the direction
 * the shut leaf lies along the wall (`ax, az`), the side it swings to
 * (`ix, iz`), its length and the angle the drafter drew it open at. The leaf
 * is drawn exactly there -- over the line on the floor, at the same angle --
 * because that line IS the door as the plan shows it, and a leaf swung to a
 * convention of our own would stand off its own symbol.
 *
 * Returns the centre of the leaf and its yaw for a box laid along +x: three's
 * `rotation.y = yaw` turns +x onto (cos yaw, 0, -sin yaw).
 *
 * @param {{hx:number, hz:number, ax:number, az:number, ix:number, iz:number, len:number, deg:number}} leaf
 * @returns {{cx:number, cz:number, yaw:number, dx:number, dz:number}}
 */
export function leafPose(leaf) {
  const t = (leaf.deg * Math.PI) / 180;
  const dx = leaf.ax * Math.cos(t) + leaf.ix * Math.sin(t);
  const dz = leaf.az * Math.cos(t) + leaf.iz * Math.sin(t);
  return {
    cx: leaf.hx + (dx * leaf.len) / 2,
    cz: leaf.hz + (dz * leaf.len) / 2,
...
/**
 * The width the model FALLS BACK to, in feet, when the confirmed rooms cannot
 * say. See src/model3d/calibrate.js for how they usually can.
 *
 * This used to be the width every building was assumed to have, with a note
 * here saying it "sets only the proportion of wall height to footprint" and
 * "cannot distort" the model. That was wrong, and it was wrong in a way that
 * took three separate complaints on one plan to see: extrudeWalls turns this
 * into feet-per-pixel, and the tracer seals gaps under 1.25ft, drops pieces
 * under 0.5ft, and types openings by their width in feet. On a plan twice this
 * wide every real distance read at half — closet doors sealed, closet walls
 * dropped, walls twice as tall against the plan as they should be.
 *
 * It is still never shown and never claimed as a measurement; every printed
 * dimension comes from the confirmed record. A published reading carries the
 * width it was actually traced at in its extent, and a visitor reads that back

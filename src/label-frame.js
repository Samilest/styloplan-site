 ...
 * it reads the wrong window. Same rule, same guard, same pass-through.
 */
export function stairsOnRender(stairs, wireframe, render) {
  const list = stairs || [];
  if (!wireframe || !render) return list;
  const from = planContentBox(wireframe);
  const to = planContentBox(render);
  ...
  return list.map((st) => {
    ...
    if (!p) return st;
    return { ...st, position: mapFrame({ x: p.x, y: p.y }, from, to) };
  });
}

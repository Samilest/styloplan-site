...
function buildDialog() {
  ...
  dialog.innerHTML = `
    <form method="dialog" class="auth-form" novalidate>
      ...
      <p class="auth-or" id="authOr"><span>or use an email address</span></p>
      ...
      <div class="fld-float" id="authPassRow">
        <input type="password" id="authPass" autocomplete="current-password" placeholder=" " required minlength="6">
        <label for="authPass">Password</label>
      </div>
      <!-- THE WAY BACK IN, under the field it is about. Before this there was
           none: a customer who had paid and forgot the password had no road
           back to their plans and credits but writing to us. Signing in only;
           creating an account has no password to forget yet. -->
      <p class="auth-forgot" id="authForgotRow"><button type="button" id="authForgot" class="link">Forgot your password?</button></p>
  ...
  const setMode = (m) => {
    mode = m;
    const up = m === 'up';
    // THE THIRD SHAPE: asking for a reset link. The same dialog with one field
    // left, because it is the same door. A separate dialog would put the email
    // they just typed somewhere they cannot see it.
    const reset = m === 'reset';
    $('authTitle').textContent = reset ? 'Reset your password' : up ? 'Create an account' : 'Sign in';
    $('authGo').textContent = reset ? 'Send the link' : up ? 'Create account' : 'Sign in';
    $('authSwap').textContent = up || reset ? 'Sign in' : 'Create an account';
    $('authAltAsk').textContent = reset ? 'Remembered it?' : up ? 'Already have an account?' : 'New to StyloPlan?';
    // One element to hide now: the wrapper carries the label with it.
    $('authNameRow').hidden = !up;
    $('authPassRow').hidden = reset;
    $('authGoogle').hidden = reset;
    $('authOr').hidden = reset;
    $('authForgotRow').hidden = m !== 'in';
    ...
    $('authAltRow').hidden = signupsOff && !reset;
  ...
  // The email already typed stays: setMode does not touch the fields.
  ...
  dialog.querySelector('form').addEventListener('submit', async (e) => {
    ...
    e.preventDefault();
    if (mode === 'reset') {
      const addr = $('authEmail').value.trim();
      if (!/^[^\s@]+@[^\s@]+$/.test(addr)) return say('Enter the email address on your account.');
      $('authGo').disabled = true;
      const label = $('authGo').textContent;
      $('authGo').textContent = 'Sending…';
      const r = await requestPasswordReset(addr);
      $('authGo').disabled = false;
      $('authGo').textContent = label;
      if (r.error) return say(r.error);
      // Worded so it claims nothing about whether the address has an account.
      return say(`If ${addr} has an account, a link to choose a new password is on its way. `
        + 'It can take a few minutes, so check your spam folder too.', 'ok');
    }
  ...
  return new Promise((resolve) => {
    ...
    d.innerHTML = `
      <form method="dialog" class="auth-form">
        ...
        <p class="subtle">Your projects, brand kits and images are kept on your
        account. You can also keep them as one file of your own.</p>
        <div class="row" style="margin-top:8px">
          <button type="button" class="btn btn--secondary btn--sm" data-act="copy">Download a copy</button>
          <button type="button" class="btn btn--quiet btn--sm" data-act="restore">Restore from a copy</button>
          <input type="file" data-act="file" accept="application/json,.json" hidden>
        </div>
        <p class="subtle" data-copy-msg role="status" hidden></p>
    ...
    // A COPY OF EVERYTHING, AS ONE FILE. This was "Back up" and "Restore" on
    // the Projects page, from when everything lived in one browser and a file
    // was the only second copy. The account keeps it all now, so the two moved
    // here, out of the way of the work (2026-09-25). Same file, same checks
    // before anything is written.
    const copyMsg = d.querySelector('[data-copy-msg]');
    const tell = (t) => { copyMsg.hidden = false; copyMsg.textContent = t; };
    d.querySelector('[data-act="copy"]').onclick = async (e) => {
      ...
      try {
        ...
        tell('Packing…');
        const blob = await exportBackup(tell);
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = `styloplan-backup-${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
        URL.revokeObjectURL(a.href);
        tell(`Copy saved (${(blob.size / 1048576).toFixed(1)} MB).`);
      } catch (err) {
        tell(`The copy could not be made: ${err.message}`);
      } finally {
        b.disabled = false;
      }
    };
    const file = d.querySelector('[data-act="file"]');
    d.querySelector('[data-act="restore"]').onclick = () => { file.value = ''; file.click(); };
    file.onchange = async () => {
      const f = file.files[0];
      if (!f) return;
      try {
        const { inspectBackup, restoreBackup } = await import('./backup.js');
        const { askConfirm } = await import('./ask.js');
        // Read and check first, and say what will be replaced BEFORE anything
        // is written. A restore replaces projects by id.
        const found = await inspectBackup(f);
        const when = found.exportedAt ? new Date(found.exportedAt).toLocaleString() : 'an unknown date';
        const go = await askConfirm({
          title: 'Restore this copy?',
          body: [
            `This copy was made on ${when}.`,
        ...
        if (!go) { tell('Restore cancelled.'); return; }
        const done = await restoreBackup(found, tell);
        tell(`Restored ${done.projects} project${done.projects === 1 ? '' : 's'}, `
          + `${done.kits} brand kit${done.kits === 1 ? '' : 's'} and `
          + `${done.images} image${done.images === 1 ? '' : 's'}. Reloading…`);
        // Every page is built from the store, so the plain way to show what
        // came back is to load the page again.
        setTimeout(() => location.reload(), 1200);
      } catch (err) {
        tell(`Restore failed: ${err.message}`);
      }
    };
    ...
    d.addEventListener('close', () => { resolve(answer); dropWhenFaded(d); });
    d.showModal();
...
/**
 * THE SECOND HALF OF A RESET: the link in the email has signed them in, and
 * this asks for the password they will use from now on.
 *
 * "Later" is allowed. They are signed in either way, and can ask for another
 * link whenever they want one. A hidden username field travels with the new
 * password so a password manager files it under the right address.
 */
function newPasswordDialog(email) {
  const d = document.createElement('dialog');
  d.className = 'auth-dialog';
  d.innerHTML = `
    <form method="dialog" class="auth-form" novalidate>
      <h3>Choose a new password</h3>
      <p class="subtle">You opened the link from your email, so you are signed in as ${esc(email)}.
      Choose the password you will use from now on.</p>
      <input type="email" autocomplete="username" value="${esc(email)}" hidden>
      <div class="fld-float" data-field>
        <input type="password" id="newPass" autocomplete="new-password" placeholder=" " minlength="6" required>
        <label for="newPass">New password</label>
      </div>
      <p class="auth-msg" data-msg role="alert" hidden></p>
      <div class="row auth-actions">
        <button type="button" class="btn btn--quiet" data-act="later">Later</button>
        <button type="submit" class="btn btn--primary" data-act="save">Save password</button>
      </div>
    </form>`;
  document.body.append(d);
  const msg = d.querySelector('[data-msg]');
  const save = d.querySelector('[data-act="save"]');
  const later = d.querySelector('[data-act="later"]');
  let done = false;
  later.onclick = () => d.close();
  d.querySelector('form').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (done) return d.close();
    save.disabled = true;
    const was = save.textContent;
    save.textContent = 'Saving…';
    const r = await setNewPassword(d.querySelector('#newPass').value);
    save.disabled = false;
    save.textContent = was;
    msg.hidden = false;
    if (r.error) { msg.className = 'auth-msg'; msg.textContent = r.error; return; }
    done = true;
    msg.className = 'auth-msg ok';
    msg.textContent = 'Your new password is saved. Use it the next time you sign in.';
    d.querySelector('[data-field]').hidden = true;
    later.hidden = true;
    save.textContent = 'Done';
    save.focus();
  });
  d.addEventListener('close', () => { clearRecovery(); dropWhenFaded(d); });
  d.showModal();
  d.querySelector('#newPass').focus();
}
...
export async function mountAccount(slot) {
  ...
  // The same once-only rule for the new-password question after a reset link.
  let shownRecovery = false;
  ...
  const draw = ({ user, credits, ready, sync, oauthError, recovery }) => {
    ...
    if (recovery && !shownRecovery) { shownRecovery = true; newPasswordDialog(user.email); }

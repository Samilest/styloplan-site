...
export async function readPublishedHome(projectId, floorId) {
  ...
  const floors = rows.map((r) => ({
    ...
    verified: {
      ...
      // case the published reading is refused (an older version) and the
      // visitor's page reads the picture itself: it then applies the same
      // decisions the builder's page does, from the same place.

...
export function dimsNotUnderName(labels, runs, transform, opts = {}) {
  ...
  for (const l of labels || []) {
    ...
    out.push({
      ...
      name: l.name,
      dim: l.dim,
      // WHAT WAS MEASURED, NOT A VERDICT ABOUT THE SHEET. The old wording was
      // "Your plan prints no size under this room name, so this figure came
      // from a dimension line elsewhere" — two claims, and neither is
      // establishable from here. All that happened is that no second line was
      // found under the mapped point, which a registration that is slightly
      // off, a name set on two lines, or an anchor that landed away from the
      // lettering will each produce on a sheet that does print the size. The
      // file's own header says this decides nothing about truth; the sentence
      // the reviewer read said otherwise, and the gate then refused to let
      // them disagree with it.
      reason: 'We could not find this size printed under the room name on your plan, '
        + 'which is where a room size usually sits. It may have come from a dimension '
        + 'line elsewhere on the sheet, or we may have looked in the wrong place. '

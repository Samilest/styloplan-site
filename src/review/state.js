...
import { scheduleRows, scheduleTotals } from '../room-schedule.js';
...
import { annotationOf } from './equipment.js';
...
export function createProject(l1) {
  project = {
    ...
    labels: l1.extraction.spaces.map((s, i) => ({
      ...
      // A transcription that is not a room's name -- "OPEN TO BELOW", a
      // ceiling note, a floor finish, an appliance code -- is an
      // annotation from the start (annotationOf), so the space count the
      // reviewer confirms is a count of spaces. One click makes it a room
      // again, as with any callout.
      ...(annotationOf(s.name) || {}),
    })),
    ...
    // `below`: which side of the ground the floor is on, true for a basement.
    // Null until the reviewer answers; the 3D view reads it to set the window
    // sill (src/model3d/level.js), and nothing printed uses it.
    specs: { beds: null, baths: null, sqft: null, below: null, confirmed: false },
    confirmedAt: null,
  };
  emit();
  return project;
...
export function updateLabel(id, patch) {
  ...
  // MOVING OR RENAMING A LABEL RETIRES ITS TICK.
  //
  // `checkedFor` records that a human looked at THIS label on THIS render --
  // Studio's whole "3 of 15 not checked yet" list rests on it, and the reason
  // it exists is that Review cannot show whether a name lands on a bed, because
  // the furniture does not exist until the render does.
  //
  // `restoreLabels` spreads the stored label, so the tick used to ride through
  // Review untouched: a label could be dragged clear across the plan and still
  // arrive in Studio claiming somebody had approved where it sits. Position and
  // name are exactly what that approval was about, so changing either takes it
  // back. Everything else about a label -- its size, its hidden flag -- leaves
  // the tick alone, because none of it moves the words.
  const moved = ('x' in patch && patch.x !== l.x) || ('y' in patch && patch.y !== l.y);
  const renamed = 'name' in patch && patch.name !== l.name;
  if (moved || renamed) delete l.checkedFor;
...
export function addStaircase(x, y) {
  ...
  // Its DIRECTION is not guessed: `up` was the default, so a stair the
  // reviewer placed printed UP on their image before they had said which way
  // it goes. Unknown until answered -- the tool asks, the marker shows it,
  // the compositor prints the arrow without a word, the 3D waits.
  const st = { id: `st${nextId++}`, kind: 'straight', printed: true,
               flights: 1, divider: false, direction: 'unknown',
...
export function suggestSpecs() {
  ...
  // ONE DEFINITION OF WHAT A DIMENSION IS. This used to carry its own regex,
  // and it disagreed with the room schedule on three real strings: a fractional
  // inch (6'-7 1/2" scored 0 instead of 66 sq ft) and a curly-quoted figure
  // (0 instead of 120) both fell out entirely, and an equipment callout with a
  // dimension on it was counted here and not there. The two numbers are shown
  // to the same person about the same floor, so they cannot come from two
  // parsers. See the warning above dimToFeet in house-style.js.
  const totals = scheduleTotals(scheduleRows(project));
  ...
  return { beds, baths, sqft: null, roomSum: totals.sum, roomsCounted: totals.counted,
           roomsMissing: totals.missing };
}
...
/**
 * The reviewer has seen the mismatch and chosen to go on anyway. `at` carries
 * an earlier acknowledgment of the same drawing forward with its own time.
 */
export function acknowledgeGeometry(at = null) {
  if (!project.geometry) return;
  project.geometry.acknowledged = true;
  project.geometry.acknowledgedAt = at || new Date().toISOString();
  emit();
}
...
/**
 * THE REVIEWER HAS THE PLAN AND WE DO NOT. `dimsNotUnderName` says it found no
 * line of text under a room's name where it looked; that is a statement about
 * what a heuristic could see through a registration transform, not about what
 * is printed on the customer's sheet. Read one way it is a useful prompt to
 * look. Read as a verdict it inverts the product: red line 2 says the numbers
 * come from the plan and from the user, and the whole gate exists to record
 * what THEY confirmed.
 *
 * So a flagged figure the reviewer stands behind is kept, and the fact that
 * they stood behind it is recorded rather than silently dropped: the
 * verification report is a document about what a person checked.
 */
export function keepDim(id) {
  const l = project.labels.find((x) => x.id === id);
  if (!l || !l.dim) return;
  l.dimKept = l.dim;                       // the exact figure that was vouched for
  l.dimKeptAt = new Date().toISOString();
  emit();
}
export function unkeepDim(id) {
  const l = project.labels.find((x) => x.id === id);
  if (!l) return;
  delete l.dimKept; delete l.dimKeptAt;
  emit();
}

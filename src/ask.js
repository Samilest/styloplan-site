...
export function askConfirm({ title, body = [], confirm = 'Confirm', cancel = 'Cancel',
  ...
  return new Promise((resolve) => {
    ...
    dlg.addEventListener('close', () => { resolve(answer); dropWhenFaded(dlg); });
...
/**
 * Take a closed dialog off the page once it has faded out. Dialogs close over
 * --d-micro (ui.css); removed on its `close` event, one was gone before its
 * fade could run. The answer is given at once -- only the element waits.
 */
export function dropWhenFaded(dlg) {
  const ms = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--d-micro')) || 120;
  setTimeout(() => dlg.remove(), ms + 60);
}

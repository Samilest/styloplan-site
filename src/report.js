...
export function renderVerificationReport({ kit, projectName, floorName, verified, page }) {
  ...
  const ink = '#2B2B2B';
  const sub = '#6b6459';
  // MARGIN, WIDENING ON A WIDE SHEET. The page takes its size from the plan it
  // travels with, and a square or landscape plan makes PAGE_W 3300 or 4400
  // design units. Holding the margin at 260 there would set every line to a
  // measure nobody can read across. The text column stays the letter-page
  // width and centres itself instead; on a portrait page this is exactly 260.
  ...
  // Only a keep that still matches the figure on the label counts: editing the
  // number after confirming it makes the confirmation about a different number.
  const kept = rooms.filter((l) => l.dimKept && l.dimKept === l.dim);
  ...
  const rows = [
    ...
    // A FIGURE THE REVIEWER WAS WARNED ABOUT AND KEPT IS A STRONGER RECORD
    // THAN ONE NOBODY QUESTIONED, so the document says so rather than letting
    // it disappear into the count above. Named, because "2 dimensions were
    // queried" without saying which is a worry with nowhere to go.
    ...(kept.length ? [[
      `${kept.length} dimension${kept.length === 1 ? ' was' : 's were'} queried and confirmed against the plan`,
      kept.map((k) => `${k.name || 'an unlabelled space'} ${k.dim}`).join(' · ')
      + '. Our check could not find these printed under the room name; the reviewer '
      + 'compared each against their own plan and kept it.',
    ]] : []),

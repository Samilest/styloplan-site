...
import { regionScale } from './sheet-part.js';
...
export async function renderPage(doc, pageNumber) {
  ...
  return paint(page, viewport, Math.round(viewport.width), Math.round(viewport.height), pageNumber);
}

/**
 * Render ONE PART of a page: the part of a sheet that is this floor.
 *
 * Rendered from the PDF again, not cut out of the whole-page image, so the part
 * gets the full resolution a whole page would (see regionScale). pdf.js draws a
 * viewport shifted by the region's corner into a canvas the region's size.
 *
 * @param {object} box  {x, y, w, h} in fractions of the page as it was shown
 */
export async function renderPageRegion(doc, pageNumber, box) {
  const page = await withTimeout(doc.getPage(pageNumber), OPEN_TIMEOUT_MS, `page ${pageNumber}`);
  const base = page.getViewport({ scale: 1 });
  const scale = regionScale(base.width, base.height, box,
    { target: TARGET_SHORT_SIDE, maxScale: MAX_SCALE, maxSide: MAX_SIDE });
  const viewport = page.getViewport({
    scale,
    offsetX: -box.x * base.width * scale,
    offsetY: -box.y * base.height * scale,
  });
  return paint(page, viewport,
    Math.max(1, Math.round(box.w * base.width * scale)),
    Math.max(1, Math.round(box.h * base.height * scale)), pageNumber);
}

/** Draw a page through `viewport` onto a white canvas of the given size. */
async function paint(page, viewport, width, height, pageNumber) {

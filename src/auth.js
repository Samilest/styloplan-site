...
function takeOAuthError() {
  ...
  for (const src of [location.hash, location.search]) {
    ...
    return { code, message: friendlyOAuth(code, decodeURIComponent(desc.replace(/\+/g, ' ')), p.get('error_code') || '') };
  }
  return null;
}
 ...
 */
export function friendlyOAuth(code, desc, errorCode = '') {
  const d = desc.toLowerCase();
  // A PASSWORD-RESET LINK THAT IS OLD OR ALREADY USED comes back as
  // error=access_denied, which the line below would call "cancelled" -- the
  // person did nothing of the kind. Checked first, on the specific code and on
  // the words, because the link is the one thing here they can simply ask for
  // again.
  if (errorCode === 'otp_expired' || /expired|invalid/.test(d) && /link|otp|token/.test(d)) {
    return 'That link has expired or was already used. Ask for a new one with "Forgot your password?".';
  }
...
/**
 * A PASSWORD-RESET LINK LANDS HERE SIGNED IN, and nothing else says so.
 *
 * Supabase sends the person back with a session in the fragment and
 * `type=recovery` beside it. supabase-js signs them in from it and announces
 * PASSWORD_RECOVERY -- but on a timer, from inside the client's own start-up,
 * which can run before anyone is listening. So the fragment is read here as
 * well, before the client exists and clears it, the same way a failed sign-in
 * is. Either one is enough.
 */
export function isRecoveryLanding(hash) {
...
export function startAuth() {
  ...
  started = (async () => {
    ...
    recovering = isRecoveryLanding(location.hash);
    const sb = await getSupabase();
    if (!sb) return publish({ user: null, credits: null, oauthError });
    // Only with a signed-in user: a recovery link that did not produce a
    // session has nothing to set a password on, and says so as an error.
    const offerNewPassword = () => {
      if (recovering && cached.user && !cached.recovery) publish({ ...cached, recovery: true });
    };
    sb.auth.onAuthStateChange((event) => {
      if (event === 'PASSWORD_RECOVERY') recovering = true;
      refresh().then(offerNewPassword);
      settleTermsAcceptance();
    });
    await refresh();
    offerNewPassword();
...
/** The new-password question has been asked; do not ask it again. */
export function clearRecovery() {
  recovering = false;
  if (cached.recovery) publish({ ...cached, recovery: false });
}
...
/**
 * Change the name the verification report signs off with.
 *
 * WHY THIS EXISTS. The name was written once, at sign-up, in a field nothing
 * validates, and there was nowhere in the app to change it afterwards. A person
 * who skipped it, or who arrived through Google with a name they do not use for
 * work, had their EMAIL printed under "Reviewed and confirmed by" on a document
 * their buyer receives, with no way to correct it.
 *
 * BOTH PLACES ARE WRITTEN because both are read. `signOffName` prefers the
 * profiles row and falls back to the account metadata; the header reads the
 * metadata. Writing one would leave the two disagreeing about who this is.
 */
export function signOffPayloads(user, fullName, now = new Date().toISOString()) {
  const name = String(fullName || '').trim();
  if (!name) return { error: 'Enter the name that should sign your reports.' };
  if (!user?.id) return { error: 'Sign in first.' };
  return {
    name,
    row: { id: user.id, full_name: name, updated_at: now },
    // THE EXISTING METADATA IS CARRIED FORWARD, not replaced. `terms_accepted_at`
    // and `terms_version` live in this object and are the evidence a chargeback
    // asks for; sending only `full_name` risks writing them away, and a silent
    // loss there would not be noticed until it mattered. Split out from the
    // call below so that rule can be tested rather than read.
    meta: { ...(user.user_metadata || {}), full_name: name },
  };
}

export async function setSignOffName(fullName) {
  const sb = await getSupabase();
  if (!sb) return { error: 'Accounts are not available here.' };
  const { data } = await sb.auth.getUser();
  const p = signOffPayloads(data?.user, fullName);
  if (p.error) return { error: p.error };

  const { error } = await sb.from('profiles').upsert(p.row);
  if (error) return { error: friendly(error.message) };
  const { error: metaError } = await sb.auth.updateUser({ data: p.meta });
  if (metaError) return { error: friendly(metaError.message) };
  return { name: p.name };
}
...
const friendly = (msg) => {
  ...
  // Supabase refuses a second reset email inside a minute with "For security
  // purposes, you can only request this after 42 seconds."
  if (/security purposes/i.test(m)) return 'A link was sent a moment ago. Wait a minute before asking for another.';
  if (/different from the old/i.test(m)) return 'That is the password you have now. Choose a new one.';
...
/**
 * WHERE A PASSWORD-RESET LINK SHOULD LAND: the Projects page, which every
 * account has and which mounts the account control that finishes the job.
 * It has to be on Supabase's redirect allowlist (Auth -> URL Configuration),
 * as Google's return already is. If it is not, Supabase sends the person to
 * the Site URL instead, and the landing page forwards a recovery link here.
 */
export const recoveryLanding = (base) => new URL('projects.html', base).href;

/**
 * Ask Supabase to email a link to choose a new password.
 *
 * THE ANSWER IS THE SAME WHETHER OR NOT THE ADDRESS HAS AN ACCOUNT. Supabase
 * replies alike for both, and the dialog says so in words that do not claim
 * either, so the form cannot be used to find out who is a customer.
 */
export async function requestPasswordReset(email) {
  const sb = await getSupabase();
  if (!sb) return { error: 'Accounts are not available here.' };
  const { error } = await sb.auth.resetPasswordForEmail(email, {
    redirectTo: recoveryLanding(document.baseURI),
  });
  if (error) return { error: friendly(error.message) };
  return {};
}

/** Set the signed-in account's password: the second half of a reset. */
export async function setNewPassword(password) {
  const sb = await getSupabase();
  if (!sb) return { error: 'Accounts are not available here.' };
  if (String(password || '').length < 6) return { error: 'Use a password of at least 6 characters.' };
 ...
 */
const PENDING_TERMS = 'pr.termsPending';

/**
 * signInWithOAuth takes no user metadata, and the page is replaced by Google's
 * before anything could be written anyway. So the acceptance is parked across
 * the redirect and written once there is a session to write it to.
 */
export function parkTermsAcceptance() {
  try { sessionStorage.setItem(PENDING_TERMS, JSON.stringify(termsRecord())); } catch { /* private mode */ }
}

/** Write a parked acceptance, and never overwrite one already on the account. */
export async function settleTermsAcceptance() {
  let parked;
  try { parked = sessionStorage.getItem(PENDING_TERMS); } catch { return; }
  if (!parked) return;
  const sb = await getSupabase();
  if (!sb) return;
  const { data } = await sb.auth.getUser();
  if (!data?.user) return;
  try {
    // The first acceptance is the one that counts. Re-writing it on every
    // later sign-in would move the date forward and destroy the evidence.
    if (!data.user.user_metadata?.terms_accepted_at) {
      await sb.auth.updateUser({ data: JSON.parse(parked) });
    }
    sessionStorage.removeItem(PENDING_TERMS);
  } catch { /* leave it parked and try again next time */ }
}
...
// WHICH TERMS WERE ACCEPTED, AND WHEN.
//
// A chargeback on a digital product is answered with evidence, and the piece
// every guide names first is proof that the customer saw and accepted the terms
// before paying. The tick in the sign-up dialog was doing nothing but gating a
// button: nothing was written down, so the record did not exist.
//
// Stored on the user rather than in a table of its own, which needs no
// migration and travels with the account. It is metadata the account owner can
// in principle edit, so it is a record we can produce rather than a signature
// they cannot repudiate; that is the honest description of it, and it is the
// same thing most products of this size actually hold.
//
// Bump this string when the terms change materially. The date is the one
// printed at the top of legal.html, so the two cannot drift apart silently.
export const TERMS_VERSION = '2026-09-07';

/** What the sign-up paths write, and what the OAuth return re-checks. */
...
export async function signUp(email, password, fullName) {
  ...
  const { data, error } = await sb.auth.signUp({
    email,
    password,
    options: { data: { full_name: fullName || '', ...termsRecord() } },
  });

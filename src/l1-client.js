...
/**
 * What a failed stage says on its own row. A refusal the person can act on
 * (sign in, top up) keeps its sentence; anything else says only that the stage
 * did not finish. The technical text went here once, a model's half-JSON
 * included, and it now lives under the failure message's Details instead.
 */
const failedDetail = (e) => (e?.actionable ? e.message : 'did not finish');
 ...
 * @param {{wireframe?: object}|null} [reuse] - a wireframe a failed run already
 *   rendered for this same file (err.partial.wireframe): kept, not drawn again
 * @returns {{ sourceUrl, wireframeUrl, extraction }}
 */
export async function runL1(file, mode, onStage, reuse = null) {
  if (mode === 'mock') return runMock(onStage);
  return runLive(file, onStage, reuse);
}
...
async function runLive(file, onStage, reuse = null) {
  onStage('read', 'run');
  ...
  const extractP = post('/api/extract').then((j) => {
    ...
    return j.extraction;
  }).catch((e) => { onStage('extract', 'error', failedDetail(e)); throw e; });
  // A WIREFRAME ALREADY DRAWN FOR THIS FILE IS KEPT (2026-09-26). When only the
  // reading failed, "Try again" used to mean choosing the file again, drawing
  // the box again, and paying for one to three image calls that had already
  // succeeded. The same framed image gives the same framing, so the drawing
  // from the failed run is exactly what this run would have asked for.
  const wireframeP = reuse?.wireframe
    ? Promise.resolve(reuse.wireframe).then((w) => { onStage('wireframe', 'done', 'kept from the last try'); return w; })
    : renderWireframe(post, framedUrl, onStage)
      .catch((e) => { onStage('wireframe', 'error', failedDetail(e)); throw e; });
  ...
  if (exSettled.status === 'rejected') {
    ...
    // The whole wireframe, not only its picture: a retry hands it back to
    // runLive, which needs its base64 for the read-back as well.
    err.partial = { wireframeUrl: wireframe.url, wireframe };   // keep the paid render recoverable
    throw err;
  }

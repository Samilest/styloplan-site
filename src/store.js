...
/**
 * The builder's decisions about the openings the 3D reads -- which are
 * windows, which are patio doors, which are garage doors, which are nothing
 * -- kept on `verified` beside the labels, because they are the same kind of
 * fact: what the person confirmed about their plan. Every reading of this
 * floor (the builder's page, the publisher, the visitor's page) applies them.
 * See applyDecisions in src/model3d/window-read.js.
 */
export function setFloorOpenings(projectId, floorId, openings) {
  const { project, floor } = getFloor(projectId, floorId);
  if (!floor?.verified) return null;
  floor.verified.openings = openings || null;
  saveProject(project);
  return floor;
}

...
//
// A CAR DRAWN ON THE SOURCE NEVER REACHES THE STYLING STEP EITHER, and this
// was measured rather than assumed (2026-09-21, Saman's question). The
// wireframe prompt names no vehicles; it strips "loose furniture ONLY if it
// is decorative (beds, sofas, rugs, plants)", and on the two sample plans
// that draw a car in the garage (Auguste 2 #6124, La Villa #3464) the model
// took the car for furniture and drew both garages empty, 2 of 2. So the
// styling prompt's premise -- the cars it draws are the only cars, added to
// a car-free wireframe -- holds, and the count for both of those garages is
// what this file reads off their dimension: one, which is what the drafter
// drew. The prompt was NOT changed for this (red line 3, and a measurement
// that shows no defect is no reason to reword a locked asset); if a plan
// ever comes back with its drawn car kept, the fix is one line in the
// wireframe prompt's furniture rule naming vehicles, re-checked the same way.
//
...
import { canonicalName, SUGGESTED_HOUSE_STYLE } from './house-style.js';
...
// NAMED AS A GARAGE: the word, or an abbreviation the app's own table reads as
// one ("GAR", "GAR 2" -- SUGGESTED_HOUSE_STYLE in house-style.js). One list:
// the image's car count and the 3D's garage door both ask here, so a floor
// that says GAR cannot get a door in the model and no car in the picture.
const namedGarage = (name) => GARAGE.test(name)
  || canonicalName(name, SUGGESTED_HOUSE_STYLE).name === 'GARAGE';

export const isGarage = (l) =>
  Boolean(l?.name && !OUTSIDE.test(l.name) && (namedGarage(l.name) || CARPORT.test(l.name)));

/**
 * A GARAGE A CAR DRIVES INTO THROUGH A DOOR: named as a garage, never a carport
 * (open, no vehicle door) and never the apron in front of one. Where the 3D
 * hangs a garage door (window-read.js, readWindows).
 */
export const hasVehicleDoor = (l) =>
  Boolean(l?.name && !OUTSIDE.test(l.name) && !CARPORT.test(l.name) && namedGarage(l.name));

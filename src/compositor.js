...
import { markAiGenerated } from './xmp.js';
...
/**
 * The smallest `label.size` that still prints at or above the legibility floor
 * in every format of the pack.
 *
 * WHY THIS IS COMPUTED AND NOT A CONSTANT. Studio's slider stopped at a fixed
 * 14 for room names, described in its own comment as "a legibility limit". It
 * is not one: `label.size` is in reference units against a 900-wide plan and
 * the compositor multiplies it by `place.w / 900`, which depends on the plan's
 * proportions and on the format. Measured, the fixed 14 is wrong in both
 * directions at once. On a wide plan it prints at 29.6px against an 18px floor,
 * so the control refused sizes that would have been perfectly legible and left
 * a builder unable to fit WALK-IN CLOSET into a walk-in closet. On a tall plan
 * it prints at 16.5px, BELOW the floor, so the compositor silently raised it
 * and the slider's number was not the size on the page.
 *
 * Taking the largest requirement across the formats is what makes one stored
 * size safe in all three: a format that places the plan smaller needs a bigger
 * reference size to reach the same printed pixels.
 *
 * @param {number} planW  the styled render's width in pixels
 * @param {number} planH  its height
 * @param {boolean} [equipment]  equipment codes have their own, lower floor
 * @returns {number} a size in `label.size` units, or 0 when the plan is unknown
 */
export function minLabelSize(planW, planH, equipment = false) {
  if (!planW || !planH) return 0;
  let need = 0;
  for (const id of Object.keys(FORMATS)) {
    const f = FORMATS[id];
    const place = planPlacement(planW, planH, f, true);
    if (!place.w) continue;
    const floorPx = minLegiblePx(f) * (equipment ? EQUIPMENT_MIN_RATIO : 1);
    need = Math.max(need, floorPx / (place.w / LABEL_REF_WIDTH));
  }
  return need;
}
...
// No word for a direction nobody has confirmed: the arrow is the plan's own
// graphic and can stand alone; UP for `unknown` was a guess printed on the
// deliverable (2026-09-21).
const dirWord = (d) => (d === 'down' ? 'DN' : d === 'up' ? 'UP' : '');
...
function drawStaircases(ctx, staircases, place, format, brand, rec) {
  ...
  for (const st of staircases) {
    ...
    const word = dirWord(st.direction);
    if (word) rec.text(ctx, word, cx, cy + below + size * 0.9);
 ...
 *
 * EVERY EXPORT LEAVES MARKED. Both helpers pass the encoded bytes through
 * markAiGenerated, which writes the IPTC trainedAlgorithmicMedia term into the
 * file's XMP. It is done HERE, in the two functions every download already goes
 * through, rather than at each call site: there are several of those in Studio
 * and a new one would otherwise ship unmarked without anybody noticing.
 * See src/xmp.js for what the requirement is and why the packet says what it
 * says. */
export async function toPngBlob(canvas) {
  const blob = await new Promise((res) => canvas.toBlob(res, 'image/png'));
  return markAiGenerated(blob);
}

export async function toWebJpgBlob(canvas, maxW = 1600, quality = 0.85) {

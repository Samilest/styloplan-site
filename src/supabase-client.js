...
export function getSupabase() {
  ...
  clientPromise = (async () => {
    // THE LIBRARY DOWNLOADS WHILE THE CONFIG ROUND TRIP IS OUT, not after it.
    // The two were serial, and on a visitor's page every request to Supabase
    // waits on this function (measured 2026-09-20: config 390ms, then the
    // library, then the RPC, then the render). A deployment with no config
    // has fetched one script for nothing, which is the same product working
    // without Supabase as before; its failure is reported where it is awaited.

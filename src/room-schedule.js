 ...
 */
import { dimToFeet, dimUnit } from './house-style.js';

const round = (n) => Math.round(n);
/** One square foot in square metres, exactly (0.3048^2). */
...
/**
 * The unit the schedule prints in: the plan's own. A plan whose every
 * dimensioned room is written in metres gets square metres; feet and inches,
 * or a mix, get square feet. Decided over the whole table, never per row, so
 * one column is one unit. Saman, 2026-09-20: "if it is needed, m^2 for a
 * metric plan".
 *
 * @param {Array} rows  scheduleRows' output
 * @returns {'sqft'|'m2'}
 */
export function scheduleUnit(rows) {
  const units = rows.map((r) => r.unit).filter(Boolean);
  return units.length && units.every((u) => u === 'm') ? 'm2' : 'sqft';
}

/** A row's area as the page prints it, in the schedule's unit. */
export const areaIn = (sqft, unit) => (sqft === null ? null : unit === 'm2' ? sqft * M2_PER_SQFT : sqft);
export const unitLabel = (unit) => (unit === 'm2' ? 'm\u00b2' : 'sq ft');
 ...
 */
export function scheduleTotals(rows, unit = scheduleUnit(rows)) {
  const withArea = rows.filter((r) => r.area !== null);
  return {
    sum: withArea.reduce((a, r) => a + round(areaIn(r.area, unit)), 0),
...
export function renderRoomSchedule({ kit, projectName, floorName, verified, page }) {
  ...
  const unit = scheduleUnit(rows);
  const totals = scheduleTotals(rows, unit);
  const areaText = (sqft) => `${round(areaIn(sqft, unit)).toLocaleString('en-US')} ${unitLabel(unit)}`;
  ...
  const note = 'Dimensions are transcribed from the plan you supplied and confirmed by you; they '
    ...
    + `rounded to the nearest ${unit === 'm2' ? 'square metre' : 'square foot'}.` + rounded
  ...
  for (const r of rows) {
    ...
    ctx.fillText(r.area === null ? '\u2014' : areaText(r.area), colArea, y);
    ctx.textAlign = 'left';
    y += 72;
  }
  ...
  if (totals.counted === 0) {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`${round(totals.sum).toLocaleString('en-US')} ${unitLabel(unit)}`, colArea, y);
    ctx.textAlign = 'left';
    y += 64;

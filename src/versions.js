...
export function snapshotOf(verified) {
  ...
  return {
    ...
    specs: {
      ...
      below: verified.specs?.below ?? null,
    },
...
export function diffVersions(a, b) {
  ...
  // The floor level is a word, not a figure, and "None" is a floor confirmed
  // before the question existed -- so it is a change only once both sides have
  // answered it, never on the first sign-off that did.
  if (typeof a.specs?.below === 'boolean' && typeof b.specs?.below === 'boolean' && a.specs.below !== b.specs.below) {
    const level = (v) => (v ? 'below ground' : 'above ground');
    out.push({ kind: 'specs', text: `Floor level: ${level(a.specs.below)} → ${level(b.specs.below)}` });
  }
  return out;
}

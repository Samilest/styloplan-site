...
export const kitToRow = (k, owner) => ({
  ...
  // THE WEBSITE RIDES THE SAME WAY (2026-09-21), and for the reason the
  // tint does: it went out as nothing, the pulled copy came back without it,
  // and on equal stamps the cloud copy wins the merge -- so the address a
  // builder typed was gone the next time they opened the page (Saman found
  // it). One more key in the jsonb, no migration, an upsert that cannot fail.
  palette: { ...(k.palette ?? {}), tint: k.planTint ?? { mode: 'off' }, ...(k.website ? { website: k.website } : {}) },
...
export const rowToKit = (r) => ({
  ...
  palette: (({ tint, website, ...rest }) => rest)(r.palette ?? {}),
  ...
  website: typeof r.palette?.website === 'string' ? r.palette.website : '',
...
// THE FLOOR'S OWN SETTINGS RIDE INSIDE `verified`, the same way the kit's tint
// rides inside `palette`: the plan title typed in Studio and the look the 3D
// geometry is pinned to (see setFloorTitle and setFloorGeoLook in store.js).
// They lived on the floor object only, and the row never carried them, so a
// title never reached a second device and a cloud-wins merge on the first one
// took it away. Both are set only once a floor is confirmed -- Studio and the
// 3D view open at that stage -- so a record to ride in always exists by then.
// A key of its own would be a hand-run migration and, until it was run, a
// failing upsert for every floor. Packed on the way out, unpacked on the way
// in; nothing else in the app sees the nesting, and the sign-off record kept
// locally is exactly what Review wrote.
export const FLOOR_SETTINGS = ['planTitle', 'geoLook'];
export function packFloorSettings(f) {
  if (!f.verified) return null;
  const carried = {};
  for (const k of FLOOR_SETTINGS) if (f[k] != null && f[k] !== '') carried[k] = f[k];
  return Object.keys(carried).length ? { ...f.verified, floorSettings: carried } : f.verified;
}
export function unpackFloorSettings(verified) {
  if (!verified) return { verified: verified ?? null, settings: {} };
  const { floorSettings, ...rest } = verified;
  const settings = {};
  for (const k of FLOOR_SETTINGS) if (floorSettings?.[k] != null) settings[k] = floorSettings[k];
  return { verified: rest, settings };
}
...
const rowToFloor = (r) => {
  const { verified, settings } = unpackFloorSettings(r.verified);
  return {
    id: r.id,
    name: r.name,
    status: r.status,

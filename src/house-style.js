...
export const SUGGESTED_HOUSE_STYLE = {
  // The abbreviations a sheet uses in place of a name (docs/plan-symbols.md,
  // tenth reading) are offered beside the spelled-out variants: BDRM 2 is
  // offered BEDROOM as BED ROOM 2 is. WC is left out on purpose -- on a
  // kitchen sheet it is a wall cabinet.
  renames: [
    { any: ['MASTER BEDROOM', 'M BEDROOM', 'M.BEDROOM', 'PRIMARY BEDROOM', 'MBR', 'PBR'], to: 'M.BEDROOM' },
    { any: ['BEDROOM', 'BED ROOM', 'BR', 'BDRM'], to: 'BEDROOM' },
    { any: ['BATH', 'BATHROOM', 'M BATH', 'M.BATH', 'POWDER', 'POWDER ROOM', 'BA', 'BTH', 'PWDR'], to: 'BATH' },
    { any: ['ENSUIT', 'ENSUITE', 'ENS', 'MBA'], to: 'ENSUITE' },
    ...
    { any: ['LR', 'LIV'], to: 'LIVING' },
    { any: ['FR', 'FAM'], to: 'FAMILY ROOM' },
    { any: ['DR', 'DIN'], to: 'DINING' },
    { any: ['KIT', 'K'], to: 'KITCHEN' },
    { any: ['PAN'], to: 'PANTRY' },
    { any: ['LAU', 'LDY'], to: 'LAUNDRY' },
    { any: ['LIN'], to: 'LINEN' },
    { any: ['GAR'], to: 'GARAGE' },
    { any: ['MUD'], to: 'MUD ROOM' },
    { any: ['OFF'], to: 'OFFICE' },
  ...
  hide: ['CLOSET', 'CL.', 'CL', 'CLO', 'CLST', 'HALLWAY', 'HALL', 'ENTRY', 'ENT', 'FOYER', 'FOY'],
  merges: [
...
// A metric pair: two numbers, each with an optional unit, joined by x, X,
// a multiplication sign, "by" or *. `3.0 x 3.3 m`, `3,60 m × 3,00 m`,
// `3000 x 3300`, `350 x 420 cm`, `3.6m x 3.0m`.
const METRIC_PAIR = /^\s*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?\s*(?:x|X|×|by|\*)\s*(\d+(?:[.,]\d+)?)\s*(mm|cm|m)?\s*$/;
...
export function dimToFeet(dim) {
  ...
  if (parts.length !== 2) return metricToFeet(straight);
  const value = (m) => {
...
/**
 * The unit a transcribed dimension is written in: 'ft' for feet and inches,
 * 'm' for metres, centimetres or millimetres, null when it is neither. The
 * room schedule prints areas in the plan's own unit (m^2 on a metric plan)
 * and asks here rather than guessing from a locale.
 *
 * @param {string} dim
 * @returns {'ft'|'m'|null}
 */
export function dimUnit(dim) {
  if (!dim) return null;
  const straight = String(dim).replace(/[\u2018\u2019\u2032]/g, "'").replace(/[\u201c\u201d\u2033]/g, '"');
  if ([...straight.matchAll(FEET_INCHES)].length === 2) return 'ft';
  return metricToFeet(straight) ? 'm' : null;
}

/**
 * The metric half of dimToFeet: a room written in metres, centimetres or
 * millimetres, as a pair of decimal feet.
 *
 * THE UNIT IS READ, THEN INFERRED BY SIZE, NEVER GUESSED BETWEEN FEET AND
 * METRES. A unit on either number names both. Without one: a bare number is
 * millimetres when it is a thousand or more (3810 is 3.81 m -- the drafting
 * convention, see docs/plan-symbols.md, eighth reading), centimetres from a
 * hundred (two of the CAD sheets, 2026-09-13), and metres when it carries a
 * decimal point (3.0 x 3.3). A small whole number with no unit -- `12 x 14`
 * -- is feet on a US listing and metres on a European one, and nothing in
 * the string says which: null, as before, rather than a room three times
 * its size. A comma before one or two digits is a decimal (3,30); before
 * three it is a thousands mark (1,050).
 *
 * @param {string} dim
 * @returns {[number, number]|null}
 */
function metricToFeet(dim) {
  const m = METRIC_PAIR.exec(dim);
  if (!m) return null;
  const num = (t) => Number(/,\d{3}$/.test(t) ? t.replace(',', '') : t.replace(',', '.'));
  const unit = m[2] || m[4] || null;
  const toFeet = (t) => {
    const n = num(t);
    if (!Number.isFinite(n) || n <= 0) return null;
    const u = unit || (n >= 1000 ? 'mm' : n >= 100 ? 'cm' : /[.,]/.test(t) ? 'm' : null);
    if (!u) return null;
    const metres = u === 'mm' ? n / 1000 : u === 'cm' ? n / 100 : n;
    return metres * FT_PER_M;
  };
  const a = toFeet(m[1]), b = toFeet(m[3]);
  return a === null || b === null ? null : [a, b];
}
